...
namespace Aran.Converters.ConverterJtsGeom
{
    public class ConvertToJtsGeo
    {
        ...
        public static ILinearRing FromRing(Aran.Geometries.Ring rng)
        {
            ...
            {
                list.Add(FromPoint(rng[i]).Coordinate);
            }
        ...
        public static IMultiLineString FromMultiLineString(Aran.Geometries.MultiLineString aranMultiLineString)
        {
            ...
            for (int i = 0; i < aranMultiLineString.Count; i++)
                lineStrings[i] = ConvertToJtsGeo.FromLineString(aranMultiLineString[i]);
        ...
        public static IPolygon FromPolygon(Aran.Geometries.Polygon aranPolygon)
        {
            ...
            ILinearRing extRng = ConvertToJtsGeo.FromRing(aranPolygon.ExteriorRing);
        ...
        {
            IPolygon[] polygons = new IPolygon[aranMultiPolygon.Count];
        ...
        private static IGeometry SimplifyGeometry(IGeometry geom, double tolerance = 0.001)
        {
            if (geom != null)
