...
{
    public partial class SuplForm : Telerik.WinControls.UI.RadForm
    {
        private int? ObjectId = null;
        public eAIPContext db;
        //internal DB.Supplement formSource = new DB.Supplement();
        internal Supplement formSource = new Supplement();
        ...
        public SuplForm(Supplement t) : this()
        {
            try
            {
                formSource = t;
        ...
        private void UniFrm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                DataEntrySort<Supplement> dataEntrySort = new DataEntrySort<Supplement> { formSource };
        ...
        void radDataEntry1_EditorInitializing(object sender, Telerik.WinControls.UI.EditorInitializingEventArgs e)
        {
            try
            {
                ...
                e.Editor.Tag = deo;
                if (e.Property?.Name.ToLowerInvariant().Contains("date") == true)
                {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                formSource = ((DataEntrySort<Supplement>)radDataEntry1.DataSource)?.FirstOrDefault();
                if (formSource?.Publicationdate > formSource?.EffectivedateFrom || formSource?.Publicationdate > formSource?.EffectivedateTo)
                {
                    ErrorLog.ShowWarning($@"Publication date can`t be later than Effective date. Please fix it and try again.");
                    return;
                }
