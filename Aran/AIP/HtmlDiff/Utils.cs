...
namespace HtmlDiff
{
    public static class Utils
    {
        ...
        private static readonly string[] SpecialCaseWordTags = { "<img", "<x:br" };

        public static bool IsTag(string item)
        {
            //if (SpecialCaseWordTags.Any(re => item != null && item.StartsWith(re))) return false;
            if (SpecialCaseWordTags.Any(re => item != null && (item.StartsWith(re) || item.EndsWith("/>")))) return false;
        ...
        public static string WrapTextUniqueId(string text, string tagName, string id, string @ref)
        {
            return $@"<{tagName} id=""{id}"" Updated=""{tagName}"" Updated-ref=""DBAUTOGROUPID{@ref}"">{text}</{tagName}>";
        }
