    ...
    public static class Static_Proc
    {
        ...
        public static List<ARINC_OBJECT> DecodeARINCFile(string[] ARINC_FILE, List<string> ListOfSectionsType, string ADHP_ICAO_CODE)
        {
                ...
                try
                {
                    ...
                    for (int i = 0; i < SectionsList.Count; i++)
                    {
                        ...
                        if (MyNode != null)
                        {
                            ...
                            {
                                string AirportIdentifier = ArenaStaticProc.GetObjectValueAsString(ArincOBJ, "Airport_ICAO_Identifier");

                                if ((AirportIdentifier == null) || (AirportIdentifier.Trim().Length == 0))
                                    AirportIdentifier = ArenaStaticProc.GetObjectValueAsString(ArincOBJ, "Airport_Identifier");
                                ...
                                if (ArincOBJ is ARINC_Airport_Primary_Record)
                                {
                                    ...
                                    if (adhp.Airport_ICAO_Identifier.StartsWith(ADHP_ICAO_CODE))
                                    {
                                        ...
                                        AreaManager.AreaUtils.WriteToAreaFile(ArenaStaticProc.GetPathToRegionsFile(), ArenaStaticProc.GetPathToSpecificationFile(), Area);
                                    }
                                }
            ...
            if (SectionCounterDictionary.Count > 0)
            {
                ...
                string filePath = ArenaStaticProc.GetPathToARINCSpecificationFile() + @"\ARINC_ResultsInfo.txt";
        ...
        private static void GetValueFromLine(XmlNodeList RecordsList, ARINC_OBJECT ArincOBJ, string ARINC_LINE)
        {
            ...
            foreach (XmlNode RecordNode in RecordsList)
            {
                ...
                ArenaStaticProc.SetObjectValue(ArincOBJ, RecordNode.SelectSingleNode("name").InnerXml.ToString(), StoredValue);
