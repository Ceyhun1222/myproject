...
namespace ARENA.Util
{
    ...
    public class DataGridViewAutoFilterColumnHeaderCell : DataGridViewColumnHeaderCell
    {
        ...
        public delegate void ColumnHeaderFilterDelegate(string ColFilter);
        public event ColumnHeaderFilterDelegate ColumnHeaderFilterChanged;  
        ...
        public DataGridViewAutoFilterColumnHeaderCell(DataGridViewColumnHeaderCell oldHeaderCell)
        {
            ...
            DataGridViewAutoFilterColumnHeaderCell filterCell = oldHeaderCell as DataGridViewAutoFilterColumnHeaderCell;
            if (filterCell != null)
            {
        ...
        private void DataGridView_ColumnSortModeChanged(object sender, DataGridViewColumnEventArgs e)
        {
            ...
            if (object.ReferenceEquals(e.Column, OwningColumn) && e.Column.SortMode == DataGridViewColumnSortMode.Automatic)
            {
        ...
        DataGridViewPaintParts paintParts)
        {
            ...
            else
            {
                ...
                if (filtered)
                {
                    graphics.DrawPolygon(SystemPens.ControlText, new Point[] { new Point(buttonBounds.Width / 2 + buttonBounds.Left - 1 + pressedOffset, buttonBounds.Height * 3 / 4 + buttonBounds.Top - 1 + pressedOffset), new Point(buttonBounds.Width / 4 + buttonBounds.Left + pressedOffset, buttonBounds.Height / 2 + buttonBounds.Top - 1 + pressedOffset), new Point(buttonBounds.Width * 3 / 4 + buttonBounds.Left - 1 + pressedOffset, buttonBounds.Height / 2 + buttonBounds.Top - 1 + pressedOffset) });
                }
                else
                {
                    graphics.FillPolygon(SystemBrushes.ControlText, new Point[] { new Point(buttonBounds.Width / 2 + buttonBounds.Left - 1 + pressedOffset, buttonBounds.Height * 3 / 4 + buttonBounds.Top - 1 + pressedOffset), new Point(buttonBounds.Width / 4 + buttonBounds.Left + pressedOffset, buttonBounds.Height / 2 + buttonBounds.Top - 1 + pressedOffset), new Point(buttonBounds.Width * 3 / 4 + buttonBounds.Left - 1 + pressedOffset, buttonBounds.Height / 2 + buttonBounds.Top - 1 + pressedOffset) });
                }
            }
        ...
        protected override void OnMouseDown(DataGridViewCellMouseEventArgs e)
        {
            ...
            if (FilteringEnabled && DropDownButtonBounds.Contains(e.X + cellBounds.Left - scrollingOffset, e.Y + cellBounds.Top))
            {
            ...
            else if (AutomaticSortingEnabled && this.DataGridView.SelectionMode != DataGridViewSelectionMode.ColumnHeaderSelect)
            {
        ...
        {
            if (!this.DataGridView.AllowUserToOrderColumns) return;
            ...
            if (sortList == null || !sortList.SupportsSorting || !AutomaticSortingEnabled)
            {
                return;
            }
            ...
            if (object.ReferenceEquals(this.DataGridView.SortedColumn, OwningColumn) && this.DataGridView.SortOrder == SortOrder.Ascending)
            {
        ...
        private void SetDropDownListBoxBounds()
        {
            ...
            Graphics graphics = dropDownListBox.CreateGraphics();
            try
            {
                //string filter = null;
            ...
            Int32 dropDownListBoxRight = dropDownListBoxLeft + dropDownListBoxWidth + 1;
            ...
            dropDownListBox.Bounds = new Rectangle(dropDownListBoxLeft, DropDownButtonBounds.Bottom, dropDownListBoxWidth, dropDownListBoxHeight);
        ...
        protected Int32 DropDownListBoxMaxHeightInternal
        {
            get
            {
                ...
                Int32 dataGridViewMaxHeight = this.DataGridView.Height - this.DataGridView.ColumnHeadersHeight - 1;
                if (this.DataGridView.DisplayedColumnCount(false) < this.DataGridView.ColumnCount)
                {
        ...
        private void DropDownListBox_LostFocus(object sender, EventArgs e)
        {
            ...
            if (DropDownButtonBounds.Contains(this.DataGridView.PointToClient(new Point(Control.MousePosition.X, Control.MousePosition.Y))))
            {
        ...
        public void DropDownListBox_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Enter:
        ...
        private void PopulateFilters()
        {
            ...
            //Debug.Assert(data != null && data.SupportsFiltering && OwningColumn != null, "DataSource is not a BindingSource, or does not support filtering, or OwningColumn is null");
            ...
            List<string> FieldsLst = (List<string>)(DataGridView).Tag;
            ArrayList list = new ArrayList(FieldsLst.Count + 1);

            list.Add((object)" ");

            for (int i = 0; i <= FieldsLst.Count - 1; i++)
            {
                //string s = ;
                list.Add((object)FieldsLst[i]);
            }
            ...
            foreach (object value in list)
            {
                ...
                //formattedValue = CStr(GetFormattedValue(value, -1, style, Nothing, Nothing, DataGridViewDataErrorContexts.Formatting))
        ...
        private void UpdateFilter()
        {
            ...
            OwningColumn.HeaderText = tbl.Columns[OwningColumn.Index].ColumnName + " " + dropDownListBox.SelectedItem.ToString();

            if (ColumnHeaderFilterChanged != null) ColumnHeaderFilterChanged(OwningColumn.HeaderText);  
            ...
            //Debug.Assert(data != null && data.SupportsFiltering, "DataSource is not an IBindingListView or does not support filtering");
        ...
        public static string GetFilterStatus(DataGridView dataGridView)
        {
            ...
            if (string.IsNullOrEmpty(data.Filter) || data == null || data.DataSource == null || !data.SupportsFiltering)
            {
                return string.Empty;
            }
        ...
        private void SetDropDownButtonBounds()
        {
            ...
            Rectangle borderRect = BorderWidths(this.DataGridView.AdjustColumnHeaderBorderStyle(this.DataGridView.AdvancedColumnHeadersBorderStyle, new DataGridViewAdvancedBorderStyle(), false, false));
            Int32 borderAndPaddingHeight = 2 + borderRect.Top + borderRect.Height + this.InheritedStyle.Padding.Vertical;
            ...
            Int32 leftOffset = default(Int32);
            if (visualStylesEnabled)
            {
                leftOffset = 3;
            }
        ...
        {
            get { return automaticSortingEnabledValue; }
            set
            {
