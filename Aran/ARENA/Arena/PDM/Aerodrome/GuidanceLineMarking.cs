...
namespace PDM
{
    public class GuidanceLineMarking : Marking
    {
        [XmlElement]
        [Browsable(false)]
        public string GuidanceLine_ID { get; set; }

        public override PDM_ENUM PDM_Type => PDM_ENUM.GuidanceLineMarking;
        ...
        public override bool StoreToDB(Dictionary<Type, ITable> AIRTRACK_TableDic)
        {
            ...
            {
                this.ExeptionDetails = new ExeptionMessage
                {
                    Message = ex.Message,
                    Source = ex.Source,
        ...
        public override void CompileRow(ref IRow row)
        {
            ...
            findx = row.Fields.FindField("ID_GuidanceLine");
            if (findx >= 0) row.set_Value(findx, this.GuidanceLine_ID);
            findx = row.Fields.FindField("FeatureGUID");
            if (findx >= 0) row.set_Value(findx, this.ID);

            findx = row.Fields.FindField("ActualDate");
            if (findx >= 0) row.set_Value(findx, this.ActualDate);
        }
    }
}
