    ...
    public class PgFilterImplementation
    {
        ...
        private string GetSqlStringForSpatialOp ( SpatialOps spatOp, int dbEntityTypeIndex, ref byte[] geomByteA  )
        {
            if ( spatOp is DWithin )
            {
                ...
                if (Geometries.Geometry.IsNullOrEmpty(dWithin.Point))
                    return string.Empty;

                GeometryWKBWriter geomWkbWriter = new GeometryWKBWriter ();
                geomWkbWriter.Write ( dWithin.Point, ByteOrder.LittleEndian );
				...
				if ( tableName == "obj_ElevatedPoint" )
					result = result.Replace ( PropInfoParser.ReplacableString,
								string.Format ( "case when ST_X(\"{0}\"::Geometry) != 'NaN' and ST_Y(\"{0}\"::Geometry) != 'NaN' then St_DWithin(\"{0}\",ST_GeomFromWKB(:geomByteA, 4326),{1}) else false end", geogColName, valueInM ) );
				else
					result = result.Replace ( PropInfoParser.ReplacableString,
            ...
            else if ( spatOp is Within )
            {
                ...
                if (Geometries.Geometry.IsNullOrEmpty(within.Geometry))
                    return string.Empty;
                ...
                #region If geom is not normal then exception occured. Added aran_is_inside stored function to fix it.
                ...
                result = result.Replace (PropInfoParser.ReplacableString,
                            string.Format ("aran_is_inside(\"{0}\", ST_GeomFromWKB(:geomByteA, 4326))", geogColName));
        ...
        private string SqlStringForSimpleValueType ( ComparisonOps compOp, ValueType valType, int dbEntityTypeIndex, AimPropInfoList propInfoList )
        {
            ...
            AimPropInfo beforeLastAimPrInfo = null;
            if (propInfoList.Count > 1)
                beforeLastAimPrInfo = propInfoList[propInfoList.Count - 2];
            string latestPropName = latestAimPropInfo.Name;
            
            if ( latestAimPropInfo.IsFeatureReference )
            {
                if (AimMetadata.IsChoice(latestAimPropInfo.TypeIndex) || (beforeLastAimPrInfo != null && AimMetadata.IsChoice(beforeLastAimPrInfo.TypeIndex)))
                ...
                case ComparisonOpType.In:
                    {
                        ...
                        IList valList = compOp.Value as IList;

                        if (valList == null || valList.Count == 0)
                        {
                            whereStr = " FALSE ";
                        }
                        else if (valList.Count > 0)
                        {
                            string valStr = ValueAsString (valType, valList [0]) [0];
                            for (int i = 1; i < valList.Count; i++)
                                valStr += "," + ValueAsString (valType, valList [i]) [0];
                            whereStr = string.Format ("\"{0}\" IN ( {1} )", latestPropName, valStr);
                        }
                        #endregion
                    }
                    break;
