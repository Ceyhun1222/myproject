...
{
    class Area3Validation:IEtodSufaceValidation
    {
        ...
        public Area3Validation(List<DrawingSurface> surfaceList)
        {
            if (surfaceList == null)
            ...
            {
                if (surface.SurfaceBase.EtodSurfaceType != EtodSurfaceType.Area3)
                    continue;
        ...
        public string GetHtmlReport(DrawingSurface surface)
        {
            ...
            if (area3 == null)
                return "";
            var resultHtml = "<div class='caption'>Area 3 Obstacles Validation</div>" +
                             "<div>" +
                             ...
                             "<div>Vertical Accuracy <=" + Common.ConvertDistance(Area3.VerticalBufferWidth, Aran.Omega.Enums.RoundType.RealValue) + "</div>" +
                             "<div>Horizontal Accuracy <=" + Common.ConvertHeight(Area3.HorizontalBufferWidth, Aran.Omega.Enums.RoundType.RealValue) + "</div>" +
                             "</div>";
            ...
            foreach (var vsDict in _vsList)
            {
                ...
                foreach (var vsPart in vs.Part)
                {
                    ...
                    if (tmpVerExtentAccuracy < 0)
                    {
                        ...
                        else if (tmpVerAccuracy > Area3.VerticalBufferWidth)
                        {
                    ...
                    else if (tmpVerExtentAccuracy > Area3.VerticalBufferWidth)
                    {
                        partHtml += "<td style='background-color:red;color:white'>" + Common.ConvertDistance(tmpVerAccuracy, Aran.Omega.Enums.RoundType.ToNearest) + " (Must be smaller than " + Common.ConvertDistance(Area3.VerticalBufferWidth, Aran.Omega.Enums.RoundType.ToNearest) + ")</td>";
                        partIsWrong = true;
                    }
                    else
                    {
                    ...
                    {
                        partHtml += "<td style='background-color:red;color:white'>" + Common.ConvertHeight(tmpHorAccuracy, Aran.Omega.Enums.RoundType.ToNearest) + " (Must be smaller than " + Common.ConvertHeight(Area3.HorizontalBufferWidth, Aran.Omega.Enums.RoundType.ToNearest) + "M)</td>";
                        partIsWrong = true;
                    }
                    else
                    {
