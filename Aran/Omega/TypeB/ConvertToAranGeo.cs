...
namespace Aran.Omega.TypeB.Topology
{
    public static class ConvertToAranGeo
    {
        ...
        public static Aran.Geometries.MultiPoint ToMultiPoint(IMultiPoint mltPoint)
        {
            var result = new Aran.Geometries.MultiPoint();
            for (int i = 0; i < mltPoint.NumPoints; i++)
            {
                IPoint esriPoint = mltPoint.GetGeometryN(i) as IPoint;
                Aran.Geometries.Point ourPoint = new Geometries.Point(esriPoint.X, esriPoint.Y, esriPoint.Z);
                result.Add(ourPoint);
            }
            return result;
