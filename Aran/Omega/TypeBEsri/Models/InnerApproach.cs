    ...
    public class InnerApproach:SurfaceBase
    {
            ...
            foreach (VerticalStructure vs in GlobalParams.AdhpObstacleList)
            {
                ...
                foreach (VerticalStructurePart vsPart in vs.Part)
                {
                    ...
                    else
                    {
                        ...
                        //var intersect = GlobalParams.GlobalParams.GeomOperators(esriGeom, extentPrj);
                        var partGeometryChoice = VerticalStructurePartGeometryChoice.ElevatedSurface;
                        //var intersect = GlobalParams.GlobalParams.GeomOperators(esriGeom, extentEsriPrj);
        ...
        public override IList<Info> PropertyList
        {
            get
            {
                ...
                _propList.Add(new Info("Length of inner edge", Common.ConvertDistance(Length, RoundType.ToNearest).ToString(), InitOmega.DistanceConverter.Unit));
                ...
                _propList.Add(new Info("Width", Common.ConvertDistance(Width, RoundType.ToNearest).ToString(), InitOmega.DistanceConverter.Unit));
