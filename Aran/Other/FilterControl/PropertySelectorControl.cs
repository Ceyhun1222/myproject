...
namespace Aran.Controls
{
    public partial class PropertySelectorControl : UserControl
    {
        ...
        public void PerformSelection(Control control, int width)
        {
            if (_classInfo == null)
                return;

            var propSelForm = new PropertySelectorForm();

            propSelForm.SelectorAddedPropInfo += SelectorAddedPropInfo;
            propSelForm.PropertySelected += PropSelForm_PropertySelected;

            propSelForm.ClassInfo = _classInfo;
            propSelForm.StartPosition = FormStartPosition.Manual;

            propSelForm.Width = width;
        ...
        private void uiEvents_propNameSelectorButton_Click (object sender, EventArgs e)
        {
            ...
            PerformSelection(control, control.Right - ui_propNameTB.Left);
        }
