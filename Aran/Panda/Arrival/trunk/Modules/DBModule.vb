...
Module DBModule
    ...
    Public Function FillRwyDirList(ByVal rwyList() As RWYType) As List(Of RunwayDirection)
        Dim length As Int32 = rwyList.Length
        Dim index As Integer
        Dim rwy As RWYType
        Dim result As List(Of RunwayDirection) = New List(Of RunwayDirection)
        For index = 0 To length - 1
            rwy = rwyList(index)
            Dim rwyDirList = pObjectDir.GetRunwayDirectionList(rwy.Identifier)
            For Each rwyDir As RunwayDirection In rwyDirList
                result.Add(rwyDir)
            Next
        Next
        Return result
            ...
            If (pRwyDRList.Count = 2) Or (Precision And (pRwyDRList.Count = 1)) Then
                ...
                For J = 0 To M - 1
                    ...
                    For K = 0 To pCenterLinePointList.Count - 1
                        ...
                        If (pRunwayCenterLinePoint.Role Is Nothing) Or (pRunwayCenterLinePoint.Location Is Nothing) Then Continue For
                    ...
                    If Not (ptDTresh Is Nothing) Then   '(RWYList(iRwyNum).pPtGeo(eRWY.PtTHR) Is Nothing) And 
                        RWYList(iRwyNum).pPtGeo(eRWY.PtTHR) = ptDTresh
                    End If

                    For P = eRWY.PtStart To eRWY.PtEnd
                        If RWYList(iRwyNum).pPtGeo(P) Is Nothing Then
                            iRwyNum = iRwyNum - 1
                            bRwyNum = False
                            Exit For
                        End If
                    ...
                    If dDT > 0 Then
                        ...
                        Else
                            ...
                            RWYList(iRwyNum).pPtGeo(eRWY.PtStart) = New Geometries.Point
                    ...
                    Else
                        ...
                        RWYList(iRwyNum).pPtGeo(eRWY.PtEnd) = New Geometries.Point
                    ...
                    For P = eRWY.PtStart To eRWY.PtEnd
                        RWYList(iRwyNum).pPtPrj(P) = ToPrj(RWYList(iRwyNum).pPtGeo(P))

                        If RWYList(iRwyNum).pPtPrj(P).IsEmpty() Then
                            iRwyNum = iRwyNum - 1
                            bRwyNum = False
                            Exit For
                        End If

                        RWYList(iRwyNum).pPtGeo(P).M = RWYList(iRwyNum).TrueBearing
                        RWYList(iRwyNum).pPtPrj(P).M = Azt2Dir(RWYList(iRwyNum).pPtGeo(P), RWYList(iRwyNum).TrueBearing)
                    Next P
					...
					End If
					If RWYList(iRwyNum).Length < 800 Then
						RWYList(iRwyNum).Index = 1
					ElseIf RWYList(iRwyNum).Length < 1200 Then
						RWYList(iRwyNum).Index = 2
					ElseIf RWYList(iRwyNum).Length < 1800 Then
						RWYList(iRwyNum).Index = 3
					Else
						RWYList(iRwyNum).Index = 4
					End If
    ...
    Function AddILSToNavList(ByVal ILS As NavaidData, ByRef NavaidList() As NavaidData) As Integer
        Dim I As Integer
        Dim N As Integer
    ...
    Function GetILSByName(ByVal ProcName As String, ByVal ADHP As ADHPType, ByRef ILS As NavaidData) As Integer
        Dim I As Integer
        Dim N As Integer
        Dim L As Integer
        Dim pos As Integer
    ...
    Public Function GetILSByNavEq(pNavaid As Navaid, ByRef ILS As NavaidData) As Integer
        Dim I As Integer
        ...
        ILS.CallSign = pNavaid.Designator
        ...
        If Not pAIXMLocalizer Is Nothing Then
            ...
            ILS.HorAccuracy = ConverterToSI.Convert(pElevPoint.HorizontalAccuracy, 0)
...
NoLocalizer:
		...
		'If ILS.Name Is Nothing Then
		ILS.Name = ILS.CallSign
		Return ILS.Index
    End Function

    Public Function GetILS(ByVal RWY As RWYType, ByRef ILS As NavaidData, ByVal Owner As ADHPType) As Integer
        Dim I As Integer
        Dim MkrCnt As Integer
...
NoLocalizer:
    ...
    Public Sub FillNavaidList(ByRef NavaidList() As NavaidData, ByRef DMEList() As NavaidData, ByRef TACANList() As NavaidData, ByRef CurrADHP As ADHPType, ByVal Radius As Double)
        Dim I As Integer
        Dim J As Integer
        ...
        For J = 0 To pNavaidList.Count - 1
            ...
            For I = 0 To pNavaid.NavaidEquipment.Count - 1
                ...
                If AixmNavaidEquipment Is Nothing Then
                    Continue For
                End If
                ...
                ElseIf NavTypeCode = eNavaidType.TACAN Then
                    ...
                    TACANList(iTACANNum).HorAccuracy = ConverterToSI.Convert(pElevPoint.HorizontalAccuracy, 0)
    ...
    Public Function FillWPT_FIXList(ByRef WPTList() As WPT_FIXType, ByVal CurrADHP As ADHPType, ByVal Radius As Double) As Integer
        ...
        Else
            ...
            'System.Array.Sort(Of WPT_FIXType)(WPTList, (delegate( a as WPT_FIXType, b As WPT_FIXType )
            '{
    ...
    Public Function GetObstaclesByPoly(ByRef ObstacleList As ObstacleContainer, ByVal pPoly As ESRI.ArcGIS.Geometry.IPolygon, ByVal fRefHeight As Double) As Integer
        Dim C As Integer
        Dim I As Integer
        Dim J As Integer
        Dim K As Integer
        ...
        Dim pGeomGeo As IGeometry
        Dim pGeomPrj As IGeometry
        ...
        ReDim ObstacleList.Parts(-1)
        ReDim ObstacleList.Obstacles(N)
        ...
        For I = 0 To N
            ...
            For J = 0 To M
                ...
                pGeomGeo.SpatialReference = pSpRefGeo
                ...
                If HorAccuracy > 0.0 Then
                    If pGeomPrj.GeometryType = ArcGIS.Geometry.esriGeometryType.esriGeometryPoint And (HorAccuracy <= 2.0) Then
                        pGeomPrj = Functions.CreatePrjCircle(pGeomPrj, HorAccuracy, 18)
                    Else
                        pTopop = pGeomPrj
                        pTopop.Simplify()
                ...
                If K > C Then
                    C += N
                    ReDim Preserve ObstacleList.Obstacles(C)
                End If
        ...
        If K >= 0 Then
            ReDim Preserve ObstacleList.Obstacles(K)
        Else
            ReDim ObstacleList.Obstacles(-1)
        End If

        Return K
    End Function

    Public Function GetObstaclesByDist(ByRef ObstacleList As ObstacleContainer, ByVal pPtPrj As ArcGIS.Geometry.IPoint, ByVal MaxDist As Double, ByVal fRefHeight As Double) As Integer
        Dim I As Integer
        Dim J As Integer
        Dim K As Integer
        Dim N As Integer
        ...
        For I = 0 To N
            ...
            'If AixmObstacle.Name = "TG0133OB" Then
            '	Z = 0.0
            'End If
            ...
            For J = 0 To M
                ...
                ObstacleList.Obstacles(K).ID = AixmObstacle.Id                      'AixmObstacle.identifier 'CStr(AixmObstacle.guid) 'AixmObstacle.designator
                ObstacleList.Obstacles(K).Identifier = AixmObstacle.Identifier
                ...
                ObstacleList.Obstacles(K).Height = Z - fRefHeight
                'ObstacleList.Obstacles(K).Dist = pProxiOperator.ReturnDistance(ObstacleList(K).pGeomPrj)
                ObstacleList.Obstacles(K).Index = K
            Next J
        Next I
    ...
    Public Function GetAnnexObstacles(ByRef ObstacleList As ObstacleContainer, ByVal adhpIdentifier As Guid, ByVal fRefHeight As Double) As Integer
        Dim I As Integer
        Dim J As Integer
        Dim K As Integer
        Dim N As Integer
        ...
        'VerticalStructureList = pObjectDir.GetVerticalStructureList(pARANPolygon)
        VerticalStructureList = pObjectDir.GetAnnexVerticalStructureList(adhpIdentifier)
        N = VerticalStructureList.Count - 1
        ...
        C = N
        K = -1
        'pProxiOperator = pPtPrj

        For I = 0 To N
            AixmObstacle = VerticalStructureList.Item(I)

            'If AixmObstacle.Name = "rix8_10" Then
            '	Z = 0.0
            'End If
            'If AixmObstacle.Name = "TG0133OB" Then
            '	Z = 0.0
    ...
    Public Function FillSegObstList(ByVal LastPoint As StepDownFIX, ByVal fRefHeight As Double, ByVal arAreaHalfWidth As Double, ByVal MOCLimit As Double, ByVal IAF_FullAreaPoly As ESRI.ArcGIS.Geometry.IPolygon, ByVal IAF_BaseAreaPoly As ESRI.ArcGIS.Geometry.IPolygon, ByRef ObstacleList As ObstacleContainer) As Integer
        Dim C As Integer
        Dim S As Integer
        Dim I As Integer
        Dim J As Integer
        ...
        Dim pFullReleation As ESRI.ArcGIS.Geometry.IRelationalOperator
        ...
        If IAF_FullAreaPoly.IsEmpty Then
            ReDim ObstacleList.Obstacles(-1)
            ReDim ObstacleList.Parts(-1)
            Return -1
        End If

        Dim pEnvelope As ArcGIS.Geometry.IEnvelope
        pEnvelope = IAF_FullAreaPoly.Envelope
        ...
        pCurrPt.PutCoords(pEnvelope.XMin, pEnvelope.YMin)
        pTmpPoints.AddPoint(pCurrPt)

        pCurrPt.PutCoords(pEnvelope.XMin, pEnvelope.YMax)
        pTmpPoints.AddPoint(pCurrPt)

        pCurrPt.PutCoords(pEnvelope.XMax, pEnvelope.YMax)
        pTmpPoints.AddPoint(pCurrPt)

        pCurrPt.PutCoords(pEnvelope.XMax, pEnvelope.YMin)
        pTmpPoints.AddPoint(pCurrPt)
        ...
        'pARANPolygon = ESRIGeometryToARANGeometry(ToGeo(IAF_FullAreaPoly))
        'DrawPolygon(IAF_FullAreaPoly, -1, ArcGIS.Display.esriSimpleFillStyle.esriSFSHorizontal)
        'Application.DoEvents()
        ...
        pFullReleation = IAF_FullAreaPoly

        FIXHeight = LastPoint.pPtPrj.Z - fRefHeight
        fDir = LastPoint.InDir
        GuidNav = LastPoint.GuidanceNav
        ...
        For I = 0 To N
            ...
            For J = 0 To M
                ...
                pGeomPrj = ToPrj(pGeomGeo)
                If pGeomPrj.IsEmpty() Then Continue For
                If pFullReleation.Disjoint(pGeomPrj) Then Continue For

                'DrawPolygon(pGeomPrj, -1, ArcGIS.Display.esriSimpleFillStyle.esriSFSForwardDiagonal)
                'While True
                '	Application.DoEvents()
                'End While
                ...
                If pGeomPrj.GeometryType = ArcGIS.Geometry.esriGeometryType.esriGeometryPoint Then
                    pObstPoints = New ESRI.ArcGIS.Geometry.Multipoint
                    pObstPoints.AddPoint(pGeomPrj)
                Else
                    pTopo = pGeomPrj
                    'pTopo.IsKnownSimple_2 = False
                    'pTopo.Simplify()

                    pObstPoints = pTopo.Intersect(IAF_FullAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry0Dimension)

                    pTmpPoints = pTopo.Intersect(IAF_FullAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry1Dimension)
                    pObstPoints.AddPointCollection(pTmpPoints)

                    If pGeomPrj.GeometryType = ArcGIS.Geometry.esriGeometryType.esriGeometryPolygon Then
                        pTmpPoints = pTopo.Intersect(IAF_FullAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry2Dimension)
                        pObstPoints.AddPointCollection(pTmpPoints)
                    End If

                    If Not IAF_BaseAreaPoly Is Nothing Then
                        pTmpPoints = pTopo.Intersect(IAF_BaseAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry0Dimension)
                        pObstPoints.AddPointCollection(pTmpPoints)

                        pTmpPoints = pTopo.Intersect(IAF_BaseAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry1Dimension)
                        pObstPoints.AddPointCollection(pTmpPoints)

                        If pGeomPrj.GeometryType = ArcGIS.Geometry.esriGeometryType.esriGeometryPolygon Then
                            pTmpPoints = pTopo.Intersect(IAF_BaseAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry2Dimension)
                            pObstPoints.AddPointCollection(pTmpPoints)
                        End If
                ...
                O = pObstPoints.PointCount - 1
                'ObstacleList.Obstacles(K).VerticalStructure = AixmObstacle 
                ReDim ObstacleList.Obstacles(K).Parts(O)
                ObstacleList.Obstacles(K).PartsNum = O + 1

                For P = 0 To O
                    pCurrPt = pObstPoints.Point(P)
                    L += 1
                    If L > S Then
                        S += N
                        ReDim Preserve ObstacleList.Parts(S)
                    End If

                    ObstacleList.Parts(L).pPtPrj = pCurrPt
                    ObstacleList.Parts(L).Owner = K
                    ObstacleList.Parts(L).Height = ObstacleList.Obstacles(K).Height
                    ObstacleList.Parts(L).Index = P
                    ObstacleList.Obstacles(K).Parts(P) = L

                    fDist = pBaseProxi.ReturnDistance(pCurrPt)
                    ObsMOC = MOCLimit * (1.0 - 2 * fDist / arAreaHalfWidth)

                    'If FIXHeight <= ObstacleList.Parts(L).Height Then
                    '	ObsR = arAreaHalfWidth
                    'Else
                    '	ObsR = arAreaHalfWidth - 0.5 * arAreaHalfWidth * (FIXHeight - ObstacleList.Parts(L).Height) / MOCLimit 'MOC '
                    'End If
                    'If ObsR > arAreaHalfWidth Then ObsR = arAreaHalfWidth

                    If GuidNav.TypeCode <> eNavaidType.DME Then
                        ObstacleList.Parts(L).Dist = Point2LineDistancePrj(pCurrPt, LastPoint.pPtPrj, fDir + 90.0)
                        ObstacleList.Parts(L).CLDist = Point2LineDistancePrj(pCurrPt, LastPoint.pPtPrj, fDir)
                    Else
                        ObstacleList.Parts(L).Dist = DegToRad(SubtractAngles(fAlpha, ReturnAngleInDegrees(GuidNav.pPtPrj, pCurrPt))) * fL
                        ObstacleList.Parts(L).CLDist = System.Math.Abs(fL - ReturnDistanceInMeters(pCurrPt, GuidNav.pPtPrj))
                    End If

                    ObstacleList.Parts(L).Flags = -CShort(fDist = 0.0)
                    ObstacleList.Parts(L).MOC = ObsMOC
                    ObstacleList.Parts(L).ReqH = ObstacleList.Obstacles(K).Height + ObsMOC
                    ObstacleList.Parts(L).hPenet = ObstacleList.Parts(L).ReqH - FIXHeight
                    ObstacleList.Parts(L).fTmp = ObstacleList.Parts(L).hPenet / ObstacleList.Parts(L).Dist
                Next P
            Next J
        Next I
        ...
        If K >= 0 Then
            ReDim Preserve ObstacleList.Obstacles(K)
            ReDim Preserve ObstacleList.Parts(L)
        Else
            ReDim ObstacleList.Obstacles(-1)
            ReDim ObstacleList.Parts(-1)
        End If
        ...
        Return P
    End Function
    Public Function FillSegObstList(ByVal LastPoint As StepDownFIX, ByVal fRefHeight As Double, ByVal IAF_FullAreaPoly As ESRI.ArcGIS.Geometry.IPolygon, ByVal IAF_BaseAreaPoly As ESRI.ArcGIS.Geometry.IPolygon, ByRef ObstacleList As ObstacleContainer) As Integer
        Return FillSegObstList(LastPoint, fRefHeight, arIFHalfWidth.Value, arIASegmentMOC.Value, IAF_FullAreaPoly, IAF_BaseAreaPoly, ObstacleList)
    End Function

    Public Function CalcIFProhibitions(ByVal LastPoint As StepDownFIX, ByVal fRefHeight As Double, ByVal arAreaHalfWidth As Double, ByVal MOCLimit As Double, ByVal IAF_FullAreaPoly As ESRI.ArcGIS.Geometry.IPolygon, ByVal IAF_BaseAreaPoly As ESRI.ArcGIS.Geometry.IPolygon, ByRef ObstacleList As ObstacleContainer, ByRef ProhibitionSectors() As IFProhibitionSector) As Integer
        Dim C As Integer
        Dim S As Integer
        Dim I As Integer
        Dim J As Integer
        ...
        pTopo.IsKnownSimple_2 = False
        pTopo.Simplify()
        'For I = 0 To IAF_FullAreaPoly.poin
        'pARANPolygon = ESRIGeometryToARANGeometry(ToGeo(IAF_FullAreaPoly))

        pARANPolygon = ESRIGeometryToARANGeometry(ToGeo(pExtendPoly))

        'DrawPolygon(IAF_FullAreaPoly, -1, ArcGIS.Display.esriSimpleFillStyle.esriSFSBackwardDiagonal)
        'Application.DoEvents()

        'DrawPolygon(pExtendPoly, -1, ArcGIS.Display.esriSimpleFillStyle.esriSFSBackwardDiagonal)
        'Application.DoEvents()
        ...
        For I = 0 To N
            ...
            For J = 0 To M
                ...
                'DrawPolygon(IAF_FullAreaPoly, -1, ArcGIS.Display.esriSimpleFillStyle.esriSFSForwardDiagonal)
                ...
                For P = 0 To O
                    pCurrPt = pObstPoints.Point(P)
                    fDist = pBaseProxi.ReturnDistance(pCurrPt)
                    ObsMOC = MOCLimit * (1.0 - 2.0 * fDist / arAreaHalfWidth)

                    'DrawPointWithText(pCurrPt, "pCurrPt")
                    'Application.DoEvents()
                    If FIXHeight - ObsHeight >= ObsMOC Then Continue For

                    If FIXHeight <= ObsHeight Then
                        ObsR = arAreaHalfWidth
                    Else
                        ObsR = arAreaHalfWidth - 0.5 * arAreaHalfWidth * (FIXHeight - ObsHeight) / MOCLimit 'MOC '
                    End If

                    If ObsR > arAreaHalfWidth Then ObsR = arAreaHalfWidth

                    If GuidNav.TypeCode <> eNavaidType.DME Then
                        ObsDist = Point2LineDistancePrj(pCurrPt, LastPoint.pPtPrj, fDir + 90.0)
                        ObsCLDist = Point2LineDistancePrj(pCurrPt, LastPoint.pPtPrj, fDir)
                    Else
                        ObsDist = DegToRad(SubtractAngles(fAlpha, ReturnAngleInDegrees(GuidNav.pPtPrj, pCurrPt))) * fL
                        ObsCLDist = System.Math.Abs(fL - ReturnDistanceInMeters(pCurrPt, GuidNav.pPtPrj))
                    End If

                    If (ObsR <= 0.5 * arAreaHalfWidth) Or (ObsCLDist > ObsR) Then Continue For
                    ...
                    If Not bAdded Then
                        ...
                        ReDim ObstacleList.Obstacles(K).Parts(O)
                        ObstacleList.Obstacles(K).PartsNum = 0
                        ...
                        ObstacleList.Obstacles(K).HorAccuracy = HorAccuracy
                        ObstacleList.Obstacles(K).VertAccuracy = VertAccuracy
                        ObstacleList.Obstacles(K).Height = ObsHeight
                        ObstacleList.Obstacles(K).Index = K
                    End If
                    ...
                    If L > S Then
                        S += N
                        ReDim Preserve ProhibitionSectors(S)
                        ReDim Preserve ObstacleList.Parts(S)
                    End If

                    ObstacleList.Parts(L).pPtPrj = pCurrPt
                    ObstacleList.Parts(L).Owner = K
                    ObstacleList.Parts(L).Height = ObsHeight
                    ObstacleList.Parts(L).Index = ObstacleList.Obstacles(K).PartsNum
                    ObstacleList.Obstacles(K).Parts(ObstacleList.Parts(L).Index) = L
                    ObstacleList.Obstacles(K).PartsNum += 1

                    ObstacleList.Parts(L).Dist = ObsDist
                    ObstacleList.Parts(L).CLDist = ObsCLDist

                    ObstacleList.Parts(L).Flags = -CShort(fDist = 0.0)

                    ObstacleList.Parts(L).MOC = ObsMOC
                    ObstacleList.Parts(L).ReqH = ObsHeight + ObsMOC
                    ObstacleList.Parts(L).hPenet = ObstacleList.Parts(L).ReqH - FIXHeight
                    ObstacleList.Parts(L).fTmp = ObstacleList.Parts(L).hPenet / ObsDist
                    ObstacleList.Parts(L).SectorIndex = L

                    ProhibitionSectors(L).rObs = ObsR
                    ProhibitionSectors(L).ObsArea = CreatePrjCircle(pCurrPt, ObsR)
                    ProhibitionSectors(L).DirToObst = ReturnAngleInDegrees(LastPoint.pPtPrj, pCurrPt)
                    ProhibitionSectors(L).DistToObst = ReturnDistanceInMeters(LastPoint.pPtPrj, pCurrPt)

                    'If ObsR < ProhibitionSectors(K).DistToObst Then
                    '	fTmp = ArcSin(rObs / ProhibitionSectors(K).DistToObst)
                    '	ProhibitionSectors(K).AlphaFrom = Round(Modulus(ProhibitionSectors(K).DirToObst - RadToDeg(fTmp), 360.0) - 0.4999)
                    '	ProhibitionSectors(K).AlphaTo = Round(Modulus(ProhibitionSectors(K).DirToObst + RadToDeg(fTmp), 360.0) + 0.4999)
                    'Else
                    '	ProhibitionSectors(K).AlphaFrom = 0
                    '	ProhibitionSectors(K).AlphaTo = 360
                    'End If

                    ProhibitionSectors(L).dHObst = ObsHeight - FIXHeight + ObsMOC
                    ProhibitionSectors(L).IndexI = L
                Next P

                If bAdded Then ReDim Preserve ObstacleList.Obstacles(K).Parts(ObstacleList.Obstacles(K).PartsNum - 1)
            Next J
        Next I

        If K >= 0 Then
            ReDim Preserve ProhibitionSectors(L)
            ReDim Preserve ObstacleList.Parts(L)
            ReDim Preserve ObstacleList.Obstacles(K)
        Else
            ReDim ProhibitionSectors(-1)
        ...
        Return L
    End Function
    Public Function CalcIFProhibitions(ByVal LastPoint As StepDownFIX, ByVal fRefHeight As Double, ByVal IAF_FullAreaPoly As ESRI.ArcGIS.Geometry.IPolygon, ByVal IAF_BaseAreaPoly As ESRI.ArcGIS.Geometry.IPolygon, ByRef ObstacleList As ObstacleContainer, ByRef ProhibitionSectors() As IFProhibitionSector) As Integer
        Return CalcIFProhibitions(LastPoint, fRefHeight, arIFHalfWidth.Value, arIASegmentMOC.Value, IAF_BaseAreaPoly, IAF_BaseAreaPoly, ObstacleList, ProhibitionSectors)
    End Function

    Public Function FillInitialObstList(ByRef ObstacleList As ObstacleContainer, ByVal FullAreaPoly As ESRI.ArcGIS.Geometry.IPolygon, Optional ByVal BaseAreaPoly As ESRI.ArcGIS.Geometry.IPolygon = Nothing) As Integer
        Dim C As Integer
        Dim S As Integer
        Dim I As Integer
        Dim J As Integer
        ...
        Dim pEnvelope As ArcGIS.Geometry.IEnvelope
        pEnvelope = FullAreaPoly.Envelope
        pExtendPoly = New ESRI.ArcGIS.Geometry.Polygon()
        pTmpPoints = pExtendPoly
        ...
        pBaseProxi = BaseAreaPoly
        pFullReleation = FullAreaPoly
        ...
        For I = 0 To N
            ...
            For J = 0 To M
                ...
                ObstacleList.Obstacles(K).Height = Z
                ObstacleList.Obstacles(K).Index = K
                ...
                Else
                    ...
                    pObstPoints = pTopo.Intersect(FullAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry0Dimension)

                    pTmpPoints = pTopo.Intersect(FullAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry1Dimension)
                    pObstPoints.AddPointCollection(pTmpPoints)

                    If pGeomPrj.GeometryType = ArcGIS.Geometry.esriGeometryType.esriGeometryPolygon Then
                        pTmpPoints = pTopo.Intersect(FullAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry2Dimension)
                        pObstPoints.AddPointCollection(pTmpPoints)
                    End If

                    If Not BaseAreaPoly Is Nothing Then
                        pTmpPoints = pTopo.Intersect(BaseAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry0Dimension)
                        pObstPoints.AddPointCollection(pTmpPoints)

                        pTmpPoints = pTopo.Intersect(BaseAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry1Dimension)
                        pObstPoints.AddPointCollection(pTmpPoints)

                        If pGeomPrj.GeometryType = ArcGIS.Geometry.esriGeometryType.esriGeometryPolygon Then
                            pTmpPoints = pTopo.Intersect(BaseAreaPoly, ArcGIS.Geometry.esriGeometryDimension.esriGeometry2Dimension)
                            pObstPoints.AddPointCollection(pTmpPoints)
                        End If
                    End If
    ...
    Public Function GetMSASectors(ByVal Nav As NavaidData, ByRef MSAList() As MSAType) As Boolean
        Dim I As Integer
        Dim J As Integer
        Dim K As Integer
        Dim M As Integer
        ...
        For I = 0 To N - 1
            ...
            If SafeAltitudeArea Is Nothing Then Continue For
            If SafeAltitudeArea.SafeAreaType Is Nothing Then Continue For

            If SafeAltitudeArea.SafeAreaType.Value = Enums.CodeSafeAltitude.MSA Then
                SAASectorList = SafeAltitudeArea.Sector
                If SAASectorList Is Nothing Then Continue For
                ...
                If M > 0 Then
                    ...
                    If pSignificantPoint Is Nothing Then Continue For
                    ...
                    For K = 0 To M - 1
                        ...
                        If SafeAltitudeSector Is Nothing Then Continue For

                        SectorDefinition = SafeAltitudeSector.SectorDefinition
                        If SectorDefinition Is Nothing Then Continue For
                        If SectorDefinition.FromAngle Is Nothing Then Continue For
                        If SectorDefinition.ToAngle Is Nothing Then Continue For
                        If SectorDefinition.AngleDirectionReference Is Nothing Then Continue For
                        If SectorDefinition.ArcDirection Is Nothing Then Continue For
                        If SectorDefinition.AngleType Is Nothing Then Continue For
                        ...
                        FromAngle = Modulus(FromAngle)
                        ToAngle = Modulus(ToAngle)
	...
	'		If TypeOf Map.Layer(I) Is ESRI.ArcGIS.Carto.IFeatureLayer Then
	...
	Public Function CreateDesignatedPoint(ByVal pPtPrj As ESRI.ArcGIS.Geometry.IPoint, Optional ByVal Name As String = "COORD", Optional ByVal fDirTreshold As Double = -1000.0, Optional ByVal horAccuracy As Double = 0.0, Optional ByVal distError As Double = 0.1) As Feature
		Dim WptFIX As WPT_FIXType
		Dim bExist As Boolean = FindDesignatedPoint(pPtPrj, WptFIX, fDirTreshold, distError)

		If bExist Then
			Dim dp As New DesignatedPoint()

			If WptFIX.TypeCode <= eNavaidType.NONE Then
				dp.Identifier = WptFIX.Identifier
			Else
				dp.Identifier = WptFIX.NAV_Ident
			End If
		...
		Return CreateNewDesignatedPoint(pPtPrj, Name, Name, horAccuracy)
	End Function

	Public Function CreateNewDesignatedPoint(ByVal pPtPrj As ESRI.ArcGIS.Geometry.IPoint, Optional ByVal Designator As String = "COORD", Optional ByVal Name As String = "COORD", Optional ByVal horAccuracy As Double = 0.0) As Feature
        Dim pFixDesignatedPoint As DesignatedPoint

        'Dim uomDistHorzTab() As UomDistance = New UomDistance() {UomDistance.KM, UomDistance.NM, UomDistance.M, UomDistance.FT, UomDistance.MI}
        'Dim mUomHDistance As UomDistance = uomDistHorzTab(DistanceUnit)

        pFixDesignatedPoint = DBModule.pObjectDir.CreateFeature(Of DesignatedPoint)()
        pFixDesignatedPoint.Designator = Designator
	...
	Public Function FindDesignatedPoint(pPtPrj As IPoint, ByRef WptFIX As WPT_FIXType, Optional ByVal fDirTreshold As Double = -1000.0, Optional ByVal distError As Double = 0.1) As Boolean
		Dim fDist As Double
		...
		If N >= 0 Then
			For I = 0 To N
				...
				If (fDist < fMinDist) And (WPTList(I).TypeCode = eNavaidType.NONE) And (WPTList(I).Name <> "COORD") Then
					fMinDist = fDist
					WptFIX = WPTList(I)
				End If
			Next I
    ...
    Public Function SlotSelector()
        ...
        Dim dbProvider As DbProvider = gAranEnv.DbProvider

        If (Not dbProvider Is Nothing And dbProvider.ProviderType = DbProviderType.TDB) Then
            Dim methodResult = New Dynamic.ExpandoObject()
            dbProvider.CallSpecialMethod("SelectSlot", methodResult)
            result = methodResult.Result
        End If

        If result = False Then
            MessageBox.Show("Please first select slot!", "Arrival", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            result = False
        End If
