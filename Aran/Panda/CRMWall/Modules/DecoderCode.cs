...
namespace Aran.PANDA.CRMWall
{
	...
	public static class DecoderCode
	{
		...
		private static string LstStDtCoder(string LST_DT, string key, int codeDir)
		{
			int[] Tab = new int[]
			...
			for (int i = 0; i < key.Length; i++)
			{
				...
				string ind = Tab[j].ToString("00");			//if (Tab[j] < 10)	ind = "0" + ind;
				decodedKey = decodedKey + ind;
			}

			return (double.Parse(LST_DT) + codeDir * double.Parse(decodedKey)).ToString("0000000000");		//while (result.Length < 10)	result = "0" + result;
		}

		public static int LstStDtWriter(string LCode, string ModuleCat, string ModuleName)
		{
			...
			string dNow = CurrData.Day.ToString("00");					//if (dNow.Length < 2) dNow = "0" + dNow;
			string mesNow = CurrData.Month.ToString("00");				//if (mesNow.Length < 2) mesNow = "0" + mesNow;
			string yrNow = (CurrData.Year - 2000).ToString("00");		//if (yrNow.Length < 2) yrNow = "0" + yrNow;

			LastStart = LstStDtCoder(dNow + mesNow + yrNow, key, 1);

			CRCCode = CRC32.CalcCRC32(LCode);
			LCode += LastStart + CRCCode;

            RegFuncs.WriteConfig(ModuleCat + "/" + ModuleName + "/" + RegFuncs.LicenseKeyName, LCode);
            return 0;
		}
		...
		public static MultiPolygon DecodeLCode(string LCode, string ModuleName)
		{
			...
			**/
			...
			//==================================================================================================
			LastStart = LstStDtCoder(LastStart, key, -1);
			...
			string CountryCode = LCode.Substring(0, 3);
			LCode = LCode.Substring(3, LCode.Length - 3);			// получили код страны

			int nFigures = int.Parse(LCode.Substring(0, 3));
			LCode = LCode.Substring(3, LCode.Length - 3);			// получили число фигур

			for (int i = 0, k = 0; k < nFigures; k++)
			{
				Ring ring;
				...
				if (FigCode != '1')
				{
					...
					int nPoints = int.Parse(LCode.Substring(i, 6));
					i += 6;						// получили число точек для данной фигуры
			...
			//GlobalVars.gAranGraphics.DrawMultiPolygon(result, -1, AranEnvironment.Symbols.eFillStyle.sfsCross);
