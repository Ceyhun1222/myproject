...
namespace Aran.PANDA.Departure
{
	...
	public static class DecoderCode
	{
		...
		private static string LastStartCoder(string LST_DT, string key, int codeDir = -1)
		{
			...
			string result = (double.Parse(LST_DT) + codeDir * double.Parse(decodedKey)).ToString();
		...
		public static int LastStartWriter(string LCode, string ModuleName)
		{
            ...
            RegFuncs.WriteConfig(GlobalVars.ModuleCategory + "/" + ModuleName + "/" + GlobalVars.LicenseKeyName, LCode);
            return 0;
		}
		...
		public static IPolygon DecodeLCode(string LCode, string ModuleName)
		{
			...
			**/
			...
			IPolygon result = new Polygon() as IPolygon;
			...
			for (int ix = 0, j = 0; j < nFigures; j++)
			{
				...
				if (FigCode != '1')
				{
					...
					int nPoints = int.Parse(LCode.Substring(ix, 6));
					ix += 6;						// получили число точек для данной фигуры

					for (int i = 0; i < nPoints; i++)
					{
						strX = LCode.Substring(ix, 7);
						strY = LCode.Substring(ix + 7, 7);
						ix += 14;										// 7 позиций X (ZAAA.BBB) + 7 позиций Y (ZAAA.BBB)
						...
						P.PutCoords(fX, fY);
						P = (IPoint)Functions.ToPrj(P);					//спроецировали точки
						if (!P.IsEmpty)
							Pol.AddPoint(P);
					}
				}
				else		//if (FigCode == '1')		// если фигура круг
				{
					...
					if (!P.IsEmpty)
					{
						...
						Pol = (IPointCollection)Functions.CreatePrjCircle(P, fR);
					}
					else
						Pol = new Polygon();
				}

				ITopologicalOperator2 pTopoOper = (ITopologicalOperator2)Pol;
				pTopoOper.IsKnownSimple_2 = false;
				pTopoOper.Simplify();

				IGeometry pGeometry = (IGeometry)Pol;
				if (!pGeometry.IsEmpty)
				{
					result = (IPolygon)pTopoOper.Union(result);

					pTopoOper = (ITopologicalOperator2)result;
					pTopoOper.IsKnownSimple_2 = false;
					pTopoOper.Simplify();
				}
			}

			//Functions.DrawPolygon(pResult, -1, ESRI.ArcGIS.Display.esriSimpleFillStyle.esriSFSCross);
			return result;
		}
	}
}
