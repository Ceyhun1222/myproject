...
namespace EOSID
{
	...
	public static class DecoderCode
	{
		...
		public static int LastStartWriter(string LCode, string ModuleName)
		{
			...
			return Functions.RegWrite(GlobalVars.PANDARootKey, GlobalVars.ModuleRegKey + "\\" + ModuleName, GlobalVars.LicenseKeyName, LCode);
		}
		...
		public static IPolygon DecodeLCode(string LCode, string ModuleName)
		{
			...
			**/
			...
			string tempS = LCode.Substring(0, prgNm * 3);						//получили название модуля в символьном виде
			string ProgName = "";
			...
			for (int j = 0, ix = 0; j < nFigures; j++)
			{
				...
				if (FigCode != '1')
				{
					...
					int nPoints = int.Parse(LCode.Substring(ix, 6));	// получили число точек для данной фигуры
					ix += 6;
					...
					for (int i = 0; i < nPoints; i++)
					{
						...
						P.PutCoords(fX, fY);
						P = (Point)Functions.ToPrj(P);						//спроецировали точки
						if (!P.IsEmpty)
							Pol.AddPoint(P);
					}
				}
				else	//if (FigCode == '1')		// если фигура круг
				{
					...
					P.PutCoords(fX, fY);
					P = (Point)Functions.ToPrj(P);
					if (!P.IsEmpty)
					{
						fR = double.Parse(strR.Substring(0, 6)) * 1000.0 + double.Parse(strR.Substring(6));
						// получили значение радиуса
						Pol = (IPointCollection)Functions.CreateCirclePrj(P, fR);
					}
					else
						Pol = new Polygon();
				}

				ITopologicalOperator2 pTopoOper = Pol as ITopologicalOperator2;
