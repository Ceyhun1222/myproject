...
namespace EOSID
{
	...
	public static class NativeMethods
	{
		...
		[DllImport("MathFunctions.dll", EntryPoint = "_ATan2@16")]	//Absolote
		public static extern double ATan2(double Y, double X);		//Absolote
		...
		public static extern int TriangleBy2PointAndAngle(double X1, double Y1, double X2, double Y2, double Angle, double h, out double xArray, out double yArray);
		[DllImport("MathFunctions.dll", EntryPoint = "_DMEcircles@60")]
		public static extern int DMEcircles(double X1, double Y1, double X2, double Y2, double alpha, out double R, out double x1res, out double y1res, out double x2res, out double y2res);
		[DllImport("MathFunctions.dll", EntryPoint = "_EnterToCircle@76")]
		public static extern int EnterToCircle(double X0, double y0, double X1, double Y1, double curAzimuth, int flag, double rRMP, double rTouch, out double xTouch, out double yTouch, out double xTurn, out double yTurn);
		[DllImport("MathFunctions.dll", EntryPoint = "_OutFromTurn@64")]
		public static extern int OutFromTurn(double X0, double y0, double X1, double Y1, double Radius, double Azimuth, int flag, out double ResX, out double ResY, out double resAzimuth);
