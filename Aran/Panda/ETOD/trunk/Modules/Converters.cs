...
namespace ETOD
{
	...
	public static class GeomConverters
	{
		...
		public static Surface ESRIPolygonToAIXMSurface(IPolygon pPolygon)
		{
			...
			IRing pRing = pExteriorRingsEnum.Next() as IRing;
			...
			while (pRing != null)
			{
				...
				IGeometryBag pInteriorRings = pPolygon4.InteriorRingBag[pRing];
				IEnumGeometry pInteriorRingsEnum = pInteriorRings as IEnumGeometry;
				pInteriorRingsEnum.Reset();

				pRing = pInteriorRingsEnum.Next() as IRing;
				while (pRing != null)
				{
					//pRing.Generalize(150);
					aRing.Clear();
					ESRIPointCollectionToARANPointCollection(pRing as IPointCollection, ref aRing);
					aPolygon.InteriorRingList.Add(aRing);
					pRing = pInteriorRingsEnum.Next() as IRing;
				}

				pSurface.Geo.Add(aPolygon);
				pRing = pExteriorRingsEnum.Next() as IRing;
			}
		...
		{
			Aran.Geometries.Box result;
			result = new Aran.Geometries.Box();
		...
		public static Aran.Geometries.Geometry ESRIGeometryToARANGeometry(IGeometry pGeom)
		{
			switch (pGeom.GeometryType)
			{
				...
				//case esriGeometryType.esriGeometryAny;			break;
				//case esriGeometryType.esriGeometryMultiPatch;	break;
				...
				//		return  ESRIPathToGeomLineString(pGeom);	break;
				//case esriGeometryType.esriGeometryBezier3Curve:
				//		return  ESRIPathToGeomLineString(pGeom);	break;
				//case esriGeometryType.esriGeometryEllipticArc:
				//		return  ESRIPathToGeomLineString(pGeom);	break;
				//case esriGeometryType.esriGeometryBag:
				//case esriGeometryType.esriGeometryTriangleStrip;		break;
