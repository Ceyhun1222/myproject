...
<ComVisibleAttribute(False)> Module DecoderCode
	...
	Private Function DeCodeChar(ByVal strVal As Char) As Char
		...
		C = System.Math.Round((C - c_CodeLen * CInt(C < 0)) * c_CodeScale)

		Return IIf(C > 0, Chr(C + s_MaxV + s_MinV * CInt(C < 11)), "-")
	End Function
...
#If OldGen Then
	Private Function Exp2(ByVal val_Renamed As Integer) As Double
		Dim I As Integer
		Dim RetVal As Double

		RetVal = 1.0
		For I = 0 To val_Renamed - 1
	...
	'Private Function DWORD2Double(ByVal val_Renamed As UInteger) As Double
	'	DWORD2Double = val_Renamed
	'	'        If val_Renamed < 0 Then DWORD2Double = DWORD2Double + 4294967296.0
	'End Function

	'Private Function DWORD2Single(ByVal Val As Long) As Single
	'    DWORD2Single = Val
	'    If Val < 0 Then DWORD2Single = DWORD2Single + 4294967296#
	'End Function
...
#End If
	...
	Public Function LstStDtWriter(ByVal LCode As String, ByVal ModuleName As String) As Integer
		...
		dNow = CStr(DateAndTime.Day(Now))
		mesNow = CStr(DateAndTime.Month(Now))
		yrNow = CStr(DateAndTime.Year(Now) - 2000)
        ...
        Aran.PANDA.Common.RegFuncs.WriteConfig(ModuleCategory + "\" + ModuleName + "\" + LicenseKeyName, LCode)
        Return 0
	End Function

	Public Function DecodeLCode(ByVal LCode As String, ByVal ModuleName As String) As Polygon
		...
		Dim T As ITopologicalOperator2
		Dim Pol As IPointCollection
		...
		If Len(LCode) <= 0 Then Return pResult

		CRCCode = Mid(LCode, Len(LCode) - 7, 8)	' получили CRC код
		LCode = Mid(LCode, 1, Len(LCode) - 8)
		...
		'DrawPolygon(pResult, RGB(255, 0, 255))
		'DrawPolygon(pResult, -1, ESRI.ArcGIS.Display.esriSimpleFillStyle.esriSFSCross)
