...
namespace Aran.PANDA.RNAV.SGBAS
{
	public partial class OASDlg : Form
	{
		...
		D3Plane _PlaneW, _PlaneWs, _PlaneX, _PlaneY, _PlaneZ;
		D3Point _PointC, _PointD, _PointE, _PointD2;			//, _PointE2 , _PointC2;
		...
		private void OASDlg_FormClosed(object sender, FormClosedEventArgs e)
		{
		...
		private void fillBoxes()
		{
			...
			oPlaneW = _PlaneW; oPlaneW1 = _PlaneWs; oPlaneX = _PlaneX; oPlaneY = _PlaneY; oPlaneZ = _PlaneZ;
			...
			if (ApproachCatBox.SelectedIndex > 2)
			{
				...
				WPrimC.Text = oPlaneW1.D.ToString("0.000");

				if (WW1InterDist > 0.0)
				{
					oPointC = Common.IntersectPlanes(oPlaneW1, oPlaneX, 0.0);

					double ZC4 = oPlaneW1.A * WW1InterDist + oPlaneW1.D;
				...
				oPointE2.Y = GlobalVars.SBASWidth;					//1852;// 
				oPointE2.X = -(oPointE2.Y * ee1.B + ee1.C) / ee1.A;
				oPointE2.Z = oPlaneZ.A * oPointE2.X + oPlaneZ.D;
			}
			else
			{
				WPrimA.Text = "";
		...
		private void CalcPlanes()
		{
			...
			//else
			{
				...
				_PlaneX.C = -1.0;
				_PlaneX.D = _PlaneX.B * (_PointD.Y * _PointC.X - _PointC.Y * _PointD.X) / (_PointD.X - _PointC.X);

				//_PointC2.Z = Common.catOASTermHeight;
				//_PointC2.X = (_PointC2.Z - _PlaneW.D) / _PlaneW.A;
				//_PointC2.Y = (_PointC2.Z - _PlaneX.A * _PointC2.X - _PlaneX.D) / _PlaneX.B;

				_PointD2.Z = Common.catOASTermHeight;
				_PointD2.X = (_PointD2.Z - Common.baseRDHHeight) / Math.Tan(GPAvalue * Math.PI * (1.0 / 180.0));
				_PointD2.Y = (_PointD2.Z - _PlaneX.A * _PointD2.X - _PlaneX.D) / _PlaneX.B;
			...
			if (ApproachCatBox.SelectedIndex > 2)
			{
				...
				_PlaneWs.A = Math.Tan(0.75 * GPAvalue * Math.PI * (1.0 / 180.0));
				_PlaneWs.B = 0.0;
				_PlaneWs.C = -1.0;
				_PlaneWs.D = _PlaneWs.A * rdhValue / Math.Tan(GPAvalue * Math.PI * (1.0 / 180.0)) - dd;
			}
