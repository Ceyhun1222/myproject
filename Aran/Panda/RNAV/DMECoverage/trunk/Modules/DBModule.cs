...
namespace Aran.PANDA.RNAV.DMECoverage
{
	...
	public static class DBModule
	{
		...
		public static int FillADHPFields(ref ADHPType CurrADHP)
		{
			...
			if (!geomOperators.Contains(GlobalVars.p_LicensePoly, pPtPrj))
				return -1;
		...
		//public static void FillNavaidList__(out NavaidType[] NavaidList, out NavaidType[] DMEList, ADHPType CurrADHP, double Radius)
		//{
		//	int i, j;
		...
		//	//GlobalVars.gAranGraphics.DrawMultiPolygon(pARANPolygon, -1, AranEnvironment.Symbols.eFillStyle.sfsDiagonalCross);
		...
		//	//for(i = 0; i< ILSDataList.Count; i++)
		//	//{
		//	//	AixmNavaidEquipment = ILSDataList.Item[i];

		//	//	if (!(AixmNavaidEquipment is ILocalizer))  continue;

		//	//	pElevPoint = AixmNavaidEquipment.ElevatedPoint;
		//	//	pGMLPoint = pElevPoint;
		//	//	pAIXMLocalizer = AixmNavaidEquipment;

		//	//	if (pAIXMLocalizer.TrueBearing == null) continue;

		//	//	pPtGeo = pGMLPoint.Tag;
		//	//	pPtGeo.Z = ConverterToSI.Convert(pElevPoint.Elevation, CurrADHP.Elev);
		...
		//	//	NavaidList[iNavaidNum].Range = 35000.0;
		//	//	NavaidList[iNavaidNum].TypeCode = eNavaidType.LLZ;
		...
		public static void FillNavaidList(out NavaidType[] NavaidList, out NavaidType[] DMEList, MultiPolygon pARANPolygon, double refHeight = 0.0, double magVar = 0.0)
		{
			int i, j;
			int iNavaidNum, iDMENum;
			...
			iNavaidNum = -1;
			iDMENum = -1;
			List<int> addedDMEs = new List<int>();
			...
			for (j = 0; j < pNavaidList.Count; j++)
			{
				...
				for (i = 0; i < pNavaid.NavaidEquipment.Count; i++)
				{
					...
					if (AixmNavaidEquipment is VOR)
						...
						NavTypeCode = eNavaidType.TACAN;                //NavTypeName = "Tacan";
					else if (AixmNavaidEquipment is Localizer || AixmNavaidEquipment is Glidepath)
					{
						inIls = true;
						continue;
					}
					...
					pPtGeo.Z = ConverterToSI.Convert(pElevPoint.Elevation, refHeight);
					pPtPrj = GlobalVars.pspatialReferenceOperation.ToPrj<Point>(pPtGeo);
					...
					if (NavTypeCode == eNavaidType.DME)
					{
						...
						DME dme = (DME)AixmNavaidEquipment;

						if (dme.Type == CodeDME.NARROW)
							DMEList[iDMENum].Tag = 1;
						else if (dme.Type == CodeDME.PRECISION)
							DMEList[iDMENum].Tag = 2;
						else if (dme.Type == CodeDME.WIDE)
							DMEList[iDMENum].Tag = 4;

						addedDMEs.Add(iDMENum);
						//DMEList[iDMENum].pFeature = pNavaid;
					}
					else
					{
						...
						if (AixmNavaidEquipment.MagneticVariation != null)
							...
							NavaidList[iNavaidNum].MagVar = magVar;
				...
				if (inIls && addedDMEs.Count > 0)
					foreach (int d in addedDMEs)
						DMEList[d].Tag |= 8;
			}
		...
		//public static int FillWPT_FIXList__(out WPT_FIXType[] WPTList, ADHPType CurrADHP, double radius)
		//{
		//	int iWPTNum, i, j, n;
  ...
  //                      continue;
		...
		//	MultiPolygon pPoly = new MultiPolygon();
		//	pPoly.Add(pPolygon);
