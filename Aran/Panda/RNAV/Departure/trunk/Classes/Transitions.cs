...
namespace Aran.PANDA.RNAV.Departure
{
	...
	public class Transitions
	{
		...
		List<LegDep> _legs;
		public List<LegDep> Legs { get { return _legs; } }
		...
		public double NomLineGradient
		{
			get
			{
				return _currFIX.NomLineGradient;
			}
			...
			set
			{
				...
				if (value > GlobalVars.constants.Pansops[ePANSOPSData.dpMaxPosPDG].Value)
					value = GlobalVars.constants.Pansops[ePANSOPSData.dpMaxPosPDG].Value;

				if (ConstructGradient < value)
					ConstructGradient = value;

				_currFIX.NomLineGradient = value;
				//_CurrLeg.NomLineGradient = value;

				double fAltitude = _referenceFIX.NomLineAltitude + (_distanceValue * NomLineGradient);

				if (_referenceFIX.NomLineAltitude < _accelerationAltitude && fAltitude > _accelerationAltitude)
				{
					double dd = (_accelerationAltitude - _referenceFIX.NomLineAltitude) / value;
					double hh = _accelerationAltitude + (_distanceValue - dd) * constants.Pansops[ePANSOPSData.dpPDG_Nom].Value;
					fAltitude = Math.Min(hh, GlobalVars.MaxAltitude);
				}
				...
				{
					_currFIX.NomLineAltitude = fAltitude;
		...
		public CodeSegmentPath PathAndTermination
        {
            ...
            set
            {
                if (_pathAndTermination != value)
                {
					...
					double minDist = CalcMinDistance(NomLineGradient);
		...
		public Transitions(List<WayPoint> SignificantCollection, Segment1Term term, RWYType RWY, double MaxDist,
			aircraftCategory category, AranEnvironment.IAranEnvironment aranEnvironment, OnInitNewFIXHandler OnInitPointHandler)
		{
			...
			_plannedTurnAngle = _term.PlannedMaxTurnAngle;
			...
			if (prevFix.FlyMode == eFlyMode.Flyby)
			{
				...
				while (i >= 0)
				{
					double hTurn = Math.Min(prevFix.NomLineAltitude + _nomLineGradientRange.Min * minDist, GlobalVars.MaxAltitude);
			...
			_nominalAltitudeRange.Max = Math.Min(GlobalVars.MaxAltitude, _nominalAltitudeRange.Min + _nomLineGradientRange.Min * _distanceValue);
			...
			NomLineGradient = _nomLineGradientRange.Min;
		...
		public bool Add(bool Close = false)
		{
			...
			_currLeg.Gradient = ConstructGradient;
			_currLeg.IAS = IAS;
		...
		void GetOstacles()
		{
			...
			if (_legs.Count == 1)
				PtPrevFIX = _prevLeg.StartFIX;

			//ObstacleContainer
			_currentDetObs = Functions.GetLegAreaObstacles(out _currentObstacleList, _currLeg, _sumMinLegLenght + _currLeg.MinLegLength,
							_RWY.pPtPrj[eRWY.ptDER].Z, _MOCLimit, PtPrevFIX);

			//if (inx >= 0)
			//	_currentDetObs = _currentObstacleList[inx];
			//else
			//	_currentDetObs = new ObstacleType();
		...
		{
			_nominalAltitudeRange.Max = Math.Min(_referenceFIX.NomLineAltitude + grd * _distanceValue, GlobalVars.MaxAltitude);
			if (_referenceFIX.NomLineAltitude <_accelerationAltitude && _nominalAltitudeRange.Max > _accelerationAltitude)
			{
			...
			if (_currFIX.FlyMode != eFlyMode.Flyby)
			{
				...
				_nominalAltitudeRange.Max = Math.Min(_referenceFIX.NomLineAltitude + grd * minDist, GlobalVars.MaxAltitude);
				return _nominalAltitudeRange.Max;
			}
			...
			do
			{
				MaxAlt = Math.Min(_nominalAltitudeRange.Min + grd * mindistOld, GlobalVars.MaxAltitude);
		...
		bool ChangeDistance(double newVal)
		{
			...
			if (Math.Abs(_distanceValue - newVal) > ARANMath.EpsilonDistance)
			{
				...
				//_NomLineGradientRange.Max = GlobalVars.constants.Pansops[ePANSOPSData.dpMaxPosPDG].Value;
				CalcMaxAltitude(_currFIX.NomLineGradient);
				...
				_constructAltitudeRange.Max = fAltitude;
				_currFIX.ConstructAltitude = fAltitude;
		...
		bool UpdateFixParams(bool updateCourseList = true)		//updateDistanceList
		{
			...
			minDist = CalcMinDistance(NomLineGradient);
