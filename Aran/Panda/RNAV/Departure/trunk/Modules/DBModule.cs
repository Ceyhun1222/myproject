...
namespace Aran.PANDA.RNAV.Departure
{
	...
	public static class DBModule
	{
			...
			for (int i = 0; i < pAIXMRWYList.Count; i++)
			{
				...
				for (int j = 0; j < pRwyDRList.Count; j++)
				{
					...
					for (int k = 0; k < pCenterLinePointList.Count; k++)
					{
						...
						switch (pRunwayCenterLinePoint.Role.Value)
						{
							case CodeRunwayPointRole.START:
								...
								RWYList[iRwyNum].clptIdent[eRWY.ptStart] = pRunwayCenterLinePoint.Identifier;
								...
								RWYList[iRwyNum].pPtPrj[eRWY.ptStart] = GlobalVars.pspatialReferenceOperation.ToPrj<Point>(RWYList[iRwyNum].pPtGeo[eRWY.ptStart]);
							...
							case CodeRunwayPointRole.THR:
								...
								RWYList[iRwyNum].clptIdent[eRWY.ptTHR] = pRunwayCenterLinePoint.Identifier;

								break;
							case CodeRunwayPointRole.DISTHR:
								ptDTresh = pElevatedPoint.Geo;
								ptDTresh.Z = ConverterToSI.Convert(pElevatedPoint.Elevation, Owner.Elev);
								idTresh = pRunwayCenterLinePoint.Identifier;
							...
							case CodeRunwayPointRole.END:
								...
								RWYList[iRwyNum].clptIdent[eRWY.ptEnd] = pRunwayCenterLinePoint.Identifier;

								RWYList[iRwyNum].pPtPrj[eRWY.ptEnd] = GlobalVars.pspatialReferenceOperation.ToPrj<Point>(RWYList[iRwyNum].pPtGeo[eRWY.ptEnd]);
					...
					if (RWYList[iRwyNum].pPtGeo[eRWY.ptTHR] == null && ptDTresh != null)
					{
						...
						RWYList[iRwyNum].clptIdent[eRWY.ptTHR] = idTresh;
					}
					...
					RWYList[iRwyNum].pPtPrj[eRWY.ptTHR] = GlobalVars.pspatialReferenceOperation.ToPrj<Point>(RWYList[iRwyNum].pPtGeo[eRWY.ptTHR]);
					...
					double facticTORA = ARANFunctions.ReturnDistanceInMeters(RWYList[iRwyNum].pPtPrj[eRWY.ptStart], RWYList[iRwyNum].pPtPrj[eRWY.ptEnd]);

					if (Math.Abs(facticTORA - fTODA) < ARANMath.EpsilonDistance && !RWYList[iRwyNum].pPtGeo[eRWY.ptEnd].IsEmpty)
					{
						RWYList[iRwyNum].pPtGeo[eRWY.ptDER] = RWYList[iRwyNum].pPtGeo[eRWY.ptEnd];
						RWYList[iRwyNum].clptIdent[eRWY.ptDER] = RWYList[iRwyNum].clptIdent[eRWY.ptEnd];
					}
					else
					{
						double ResX, ResY;
						...
						RWYList[iRwyNum].clptIdent[eRWY.ptDER] = Guid.Empty;
					}

					RWYList[iRwyNum].pPtPrj[eRWY.ptDER] = GlobalVars.pspatialReferenceOperation.ToPrj<Point>(RWYList[iRwyNum].pPtGeo[eRWY.ptDER]);
					...
					//double fTmp1 = ARANFunctions.ReturnDistanceInMeters(RWYList[iRwyNum].pPtPrj[eRWY.ptStart], RWYList[iRwyNum].pPtPrj[eRWY.ptEnd]);
		...
		//	for (int i = 0; i < n; i++)
		//		if (GlobalVars.RWYList[i].Name == RWYName)
		//			return GetILS(GlobalVars.RWYList[i], ref ILS, ADHP);
		...
		public static Feature CreateDesignatedPoint(WayPoint wpt)           //Point pPtPrj, string Name = "COORD", double fDirInRad = -1000.0, double horAccuracy = 0.0
		{
