...
namespace Aran.PANDA.RNAV.Enroute.VD
{
	...
	public static class DBModule
	{
		...
		public static void FillNavaidList(out NavaidType[] NavaidList, out NavaidType[] DMEList, MultiPolygon searchArea)
		{
			const double collocatedMaxDist = 2000.0 * 0.3048;
			const double DoplerRange = 75.0 * 1852.0;
			const double ConventionalRange = 60.0 * 1852.0;
			...
			MultiPolygon pARANPolygon = GlobalVars.pspatialReferenceOperation.ToGeo<MultiPolygon>(searchArea);
			...
			for (j = 0; j < pNavaidList.Count; j++)
			{
				...
				if (pNavaid.NavaidEquipment.Count < 2)
					continue;
				...
				for (i = 0; i < pNavaid.NavaidEquipment.Count; i++)
				{
					...
					else if (NavTypeCode == eNavaidType.VOR)
					{
						...
						if (((Aran.Aim.Features.VOR)AixmNavaidEquipment).Type == CodeVOR.DVOR)
						{
							NavaidList[iNavaidNum].TypeCode = NavTypeCode | eNavaidType.Dopler;
							NavaidList[iNavaidNum].Range = DoplerRange;			//DVOR.Range
						}
						else
						{
							NavaidList[iNavaidNum].TypeCode = NavTypeCode;
							NavaidList[iNavaidNum].Range = ConventionalRange;		//VOR.Range
						}
			...
			while (i < iNavaidNum)
			{
				if (NavaidList[i].PairNavaidIndex < 0)
				{
					Array.Copy(NavaidList, i + 1, NavaidList, i, iNavaidNum - i-1);
					iNavaidNum--;
					continue;
				}
				i++;
			...
			while (i < iDMENum)
			{
				if (DMEList[i].PairNavaidIndex < 0.0)
				{
					Array.Copy(DMEList, i + 1, DMEList, i, iDMENum - i-1);
					iDMENum--;
					continue;
				}
				i++;
			...
			for (j = 0; j < iNavaidNum; j++)
				for (i = 0; i < iDMENum; i++)
				{
					...
					if (fDist <= collocatedMaxDist)
					{
						NavaidList[j].PairNavaidIndex = i;
						DMEList[i].PairNavaidIndex = j;
						DMEList[i].Range = NavaidList[j].Range;
						break;
					}
				}


			System.Array.Resize<NavaidType>(ref NavaidList, iNavaidNum);
			System.Array.Resize<NavaidType>(ref DMEList, iDMENum);
		}
		...
		//public static int GetObstaclesByPoly(out Obstacle[] ObstacleList, MultiPolygon pPoly, double fRefHeight = 0.0)
		//{
		//	int i, j, n;
		...
		//	ObstacleList = new Obstacle[0];

		//	if (n == 0)
		//		return -1;

		//	System.Array.Resize<Obstacle>(ref ObstacleList, n);
		//	j = -1;
		...
		//		j++;
		//		ObstacleList[j].pGeomGeo = pPtGeo;
		//		ObstacleList[j].pGeomPrj = pPtPrj;
		...
		//		ObstacleList[j].TypeName = AixmObstacle.Type.ToString();
		//		ObstacleList[j].UnicalName = AixmObstacle.Name;
		...
		//static public int GetObstaclesByDist(out Obstacle[] ObstacleList, Point ptCenter, double MaxDist, double fRefHeight = 0.0)
		//{
		//	int i, j, n;
		...
		//public static int GetObstaclesByDist(out Aran.PANDA.Common.ObstacleContainer ObstacleList, Point ptCenter, double MaxDist, double fRefHeight = 0.0)
		//{
		...
		//			//ObstacleList.Obstacles[l].pFeature = AixmObstacle;
		//			ObstacleList.Obstacles[l].UnicalName = aixmObstacle.Name;
		//			ObstacleList.Obstacles[l].TypeName = aixmObstacle.Type == null ? "" : aixmObstacle.Type.ToString();
		...
		static public int GetLegObstList(ObstacleContainer inObstacleList, ref Segment currLeg, double fRefHeight = 0.0)
		{
			int n = inObstacleList.Obstacles.Length;

			ObstacleContainer outObstacleList;

			outObstacleList.Obstacles = new Obstacle[n];
			...
			if (n == 0)
			{
				currLeg.Obstacles = outObstacleList;
				return 0;
			}

			//================================================================================================================

			Geometries.Operators.JtsGeometryOperators fullGeoOp = new Geometries.Operators.JtsGeometryOperators(),
				primaryGeoOp = new Geometries.Operators.JtsGeometryOperators(),
				lineStrGeoOp = new Geometries.Operators.JtsGeometryOperators();

			fullGeoOp.CurrentGeometry = currLeg.leg.FullAssesmentArea;				//	.SecondaryProtectionArea;
			primaryGeoOp.CurrentGeometry = currLeg.leg.PrimaryAssesmentArea;        //	.PrimaryProtectionArea;
																					//lineStrGeoOp.CurrentGeometry = currLeg.leg.AssesmentAreaOutline;        //	 ARANFunctions.PolygonToPolyLine(currLeg.SecondaryProtectionArea);
			lineStrGeoOp.CurrentGeometry = currLeg.leg.FullProtectionAreaOutline();	//ARANFunctions.PolygonToPolyLine(currLeg.leg.FullAssesmentArea);
			...
			for (int i = 0; i < n; i++)
			{
				...
				bool bHaveInterseck = (!fullGeoOp.Disjoint(pGeomPrj));
				if (!bHaveInterseck)
					continue;
				...
				outObstacleList.Obstacles[l] = inObstacleList.Obstacles[i];
				outObstacleList.Obstacles[l].PartsNum = 0;
				//outObstacleList.Obstacles[l].Parts = new int[p];

				for (int j = 0; j < p; j++)
				{
					Point ptCurr = pObstPoints[j];

					if (!fullGeoOp.Disjoint(ptCurr))
					{
						k++;
						if (k >= c)
						{
							c += n;
							System.Array.Resize<ObstacleData>(ref outObstacleList.Parts, c);
						}
						...
						//outObstacleList.Obstacles[l].Parts[outObstacleList.Obstacles[l].PartsNum] = k;
						outObstacleList.Obstacles[l].PartsNum++;

						ARANFunctions.PrjToLocal(currLeg.Start.PrjPt, currLeg.Start.OutDirection, ptCurr, out outObstacleList.Parts[k].Dist, out outObstacleList.Parts[k].CLShift);

						double distToPrimaryPoly = primaryGeoOp.GetDistance(ptCurr);
						outObstacleList.Parts[k].Prima = distToPrimaryPoly == 0;                                                   // <= outForwObstacleList.Obstacles[lf].HorAccuracy;
						...
						if (!outObstacleList.Parts[k].Prima)
						{
							...
							outObstacleList.Parts[k].fSecCoeff = (d1) / d;                                                         // + outForwObstacleList.Obstacles[lf].HorAccuracy

							if (outObstacleList.Parts[k].fSecCoeff > 1.0)
							{
								outObstacleList.Parts[k].fSecCoeff = 1.0;
								outObstacleList.Parts[k].Prima = true;
							}
						}

						outObstacleList.Parts[k].MOC = outObstacleList.Parts[k].fSecCoeff * MOCLimit;
						outObstacleList.Parts[k].ReqH = outObstacleList.Parts[k].MOC + outObstacleList.Parts[k].Height;  // + inObstacleList.Obstacles[l].VertAccuracy;

						if (MOCA < outObstacleList.Parts[k].ReqH)
							MOCA = outObstacleList.Parts[k].ReqH;
						outObstacleList.Parts[k].Ignored = false;
					}

				}

				//Array.Resize<int>(ref outObstacleList.Obstacles[l].Parts, outObstacleList.Obstacles[l].PartsNum);

				if (outObstacleList.Obstacles[l].PartsNum > 0)
					l++;
			}


			Array.Resize(ref outObstacleList.Obstacles, l);
			Array.Resize(ref outObstacleList.Parts, k + 1);
		...
		public static Feature CreateDesignatedPoint(Point pPtPrj, string Name = "COORD", double fDirInRad = -1000.0)
		{
			...
			if (bExist)
				return WptFIX.GetFeature();	//	WptFIX.GetSignificantPoint();// 
