...
Module DBModule
	...
	'Public Function CreateGMLCircle(ByRef ptCenterGeo As ESRI.ArcGIS.Geometry.IPoint, ByRef Radius As Double) As GMLPolygon
	'	Dim pGMLPoint As IGMLPoint
	'	Dim pGMLRing As IGMLRing
	'	Dim pGMLPoly As IGMLPolygon

	'	pGMLPoint = New GMLPoint
	'	pGMLPoint.X = ptCenterGeo.X	'Modulus(ptCenterGeo.X, 360.0)
	'	pGMLPoint.Y = ptCenterGeo.Y

	'	pGMLRing = New GMLRing
	'	pGMLPoly = New GMLPolygon
	...
	'	CreateGMLCircle = pGMLPoly
	'End Function

	'   Public Function CreateGMLCircleP(ByRef ptCenterPRJ As ESRI.ArcGIS.Geometry.IPoint, ByRef Radius As Double) As GMLPolygon
	'       Dim ptCenterGeo As ESRI.ArcGIS.Geometry.IPoint
	'       ptCenterGeo = ToGeo(ptCenterPRJ)
	'       CreateGMLCircleP = CreateGMLCircle(ptCenterGeo, Radius)
	'   End Function

	'   Public Function ConvertToGMLPoly(ByRef pPolyGEO As ESRI.ArcGIS.Geometry.IPolygon) As GMLPolygon
	'       Dim I As Integer
	'       Dim J As Integer
	'       Dim N As Integer
	'       Dim M As Integer
	...
	'       ConvertToGMLPoly = Nothing
	'       If pPolyGEO Is Nothing Then Exit Function

	'       pGMLPoly = New GMLPolygon
	'       ConvertToGMLPoly = pGMLPoly

	'       N = pPolyGEO.ExteriorRingCount
	'       If N = 0 Then Exit Function

	'       ReDim pExteriorRing(N - 1)
	...
	'           pPointCol = pExteriorRing(I)
	'           M = pPointCol.PointCount
	'           dX = System.Math.Abs(pPointCol.Point(0).X - pPointCol.Point(M - 1).X)
	'           dY = System.Math.Abs(pPointCol.Point(0).Y - pPointCol.Point(M - 1).Y)
	'           If dX * dY < 0.001 Then M = M - 1

	'           For J = 0 To M - 1
	'               pGMLPoint = New GMLPoint
	'               pPoint = pPointCol.Point(J)
	'               pGMLPoint.X = pPoint.X 'Modulus(pPoint.X, 360.0)
	...
	'   Public Function ConvertToGMLPolyP(ByRef pPolyPRJ As ESRI.ArcGIS.Geometry.IPolygon) As GMLPolygon
	'       Dim pPolyGEO As ESRI.ArcGIS.Geometry.IPolygon

	'       pPolyGEO = ToGeo(pPolyPRJ)
	'       ConvertToGMLPolyP = ConvertToGMLPoly(pPolyGEO)
	'   End Function

	'   Public Function GmlPointToIPoint(ByRef gmlPoint As IGMLPoint) As ESRI.ArcGIS.Geometry.IPoint
	'       GmlPointToIPoint = New ESRI.ArcGIS.Geometry.Point
	'       GmlPointToIPoint.PutCoords(gmlPoint.X, gmlPoint.Y)
	'   End Function
	...
	Public Function FillADHPFields(ByRef CurrADHP As ADHPType) As Integer
		...
		If pADHP Is Nothing Then
			ShowErrorMessage("Error on GetFeature [FillADHP - Identifier: " + CurrADHP.Identifier.ToString() + "]")
			Return -1
		End If
	...
	Public Function FillRWYList(ByRef RWYList() As RWYType, ByRef Owner As ADHPType) As Integer
			...
			If pRwyDRList.Count = 2 Then
				...
				For J = 0 To M - 1
					...
					For K = 0 To pCenterLinePointList.Count - 1
						...
						If (Not pRunwayCenterLinePoint.Role Is Nothing) Then
	...
	Function GetRWYByProcedureName(ByRef ProcName As String, ByRef ADHP As ADHPType, ByRef RWY As RWYType) As Boolean
		Dim I As Integer
		Dim N As Integer
		Dim Pos As Integer
...
NoLocalizer:
    ...
    Public Function FillWPT_FIXList(ByRef WPTList() As WPT_FIXType, ByRef CurrADHP As ADHPType, ByRef Radius As Double) As Integer
	...
	Public Function GetObstaclesByPoly(ByRef ObstacleList() As ObstacleType, ByRef pPoly As ESRI.ArcGIS.Geometry.IPolygon, Optional ByVal fRefHeight As Double = 0.0) As Integer
		Dim I As Integer
		Dim J As Integer
		Dim K As Integer
		Dim N As Integer
		...
		For I = 0 To N
			...
			For J = 0 To M
				...
				K += 1
				If K = UBound(ObstacleList) Then
					ReDim Preserve ObstacleList(K + 0.5 * K)
				End If

				ObstacleList(K).pPtGeo = pGeomGeo
				ObstacleList(K).pPtPrj = pGeomPrj
				...
				ObstacleList(K).Height = ObstacleList(K).pPtGeo.Z - fRefHeight
	...
	Public Function GetObstaclesByDist(ByRef ObstacleList() As ObstacleType, ByRef CurrADHP As ADHPType, ByRef MaxDist As Double, Optional ByVal fRefHeight As Double = 0.0) As Integer
		Dim pPolygon As ESRI.ArcGIS.Geometry.IPolygon
		pPolygon = CreatePrjCircle(CurrADHP.pPtPrj, MaxDist)

		Return GetObstaclesByPoly(ObstacleList, pPolygon, fRefHeight)
	End Function

	Public Function GetMSASectors(ByRef Nav As NavaidType, ByRef MSAList() As MSAType) As Boolean
		Dim I As Integer
		Dim J As Integer
		Dim K As Integer
		Dim M As Integer
	...
	'Private Function CreateMSASectorPrj(ByRef ptCntGeo As ESRI.ArcGIS.Geometry.IPoint, ByRef Sector As SectorMSA) As ESRI.ArcGIS.Geometry.Polygon
	'	Dim I As Integer
	'	Dim N As Integer
	...
	Public Function CreateDesignatedPoint(ByVal pPtPrj As ESRI.ArcGIS.Geometry.IPoint, Optional ByVal Name As String = "COORD", Optional ByVal fDir As Double = -1000.0) As DesignatedPoint
		...
		If (Not bExist) And (fMinDist <= 100.0) And (fDir <> -1000.0) Then
			fDirToPt = ReturnAngleInDegrees(pPtPrj, WptFIX.pPtPrj)
			bExist = SubtractAngles(fDir, fDirToPt) < 0.1
		End If
