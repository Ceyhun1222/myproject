...
namespace PDM
{
    ...
    public class GlidePath : NavaidComponent
    {
        ...
        private double _antennaPosition;
        ...
        public override string TypeName
        {
            get
            {
                return PDM_ENUM.GlidePath.ToString();
            }
        } 
        //[Browsable(false)]
        ...
        public override void CompileRow(ref IRow row)
        {
            ...
            findx = row.Fields.FindField("antennaPosition"); if (findx >= 0) row.set_Value(findx, this.AntennaPosition);
            findx = row.Fields.FindField("angle"); if (findx >= 0) row.set_Value(findx, this.Angle);
            findx = row.Fields.FindField("thresholdCrossingHeight"); if (findx >= 0) row.set_Value(findx, this.ThresholdCrossingHeight);
