...
namespace Aran.Temporality.Common.ArcGis.Wkt
{
    public static class WktGeometryExtensions
    {
        ...
        public static IGeometry ToGeometry(this string wkt, bool simplify=true)
        {
            return BuildGeometry(wkt, CreateDefaultSpatialReference(), simplify);
        }

        public static IGeometry ToGeometry(this string wkt, ISpatialReference spatialReference, bool simplify)
        {
            return BuildGeometry(wkt, spatialReference, simplify);
        }
        ...
        private static IGeometry BuildGeometry(string s, ISpatialReference spatialReference, bool simplify)
        {
            ...
            switch (wkt.Type)
            {
                ...
                case WktType.LineString:
                    return BuildPolyline(wkt, spatialReference, simplify);
                case WktType.Polygon:
                    return BuildPolygon(wkt, spatialReference, simplify);
                case WktType.Triangle:
                    return BuildPolygon(wkt, spatialReference, simplify);
                case WktType.PolyhedralSurface:
                    return BuildMultiPatch(wkt, spatialReference, simplify);
                case WktType.Tin:
                    return BuildMultiPolygon(wkt, spatialReference, simplify);
                case WktType.MultiPoint:
                    return BuildMultiPoint(wkt, spatialReference, simplify);
                case WktType.MultiLineString:
                    return BuildMultiPolyline(wkt, spatialReference, simplify);
                case WktType.MultiPolygon:
                    return BuildMultiPolygon(wkt, spatialReference, simplify);
                case WktType.GeometryCollection:
                    return BuildGeometryCollection(wkt, spatialReference, simplify);
                default:
                    throw new ArgumentOutOfRangeException(nameof(s), @"Unsupported geometry type: " + wkt.Type);
            }
        }
        ...
        private static IGeometry BuildMultiPoint(WktText wkt, ISpatialReference spatialReference, bool simplify)
        {
        ...
        private static IGeometry BuildPolyline(WktText wkt, ISpatialReference spatialReference, bool simplify)
        {
        ...
        private static IGeometry BuildMultiPolyline(WktText wkt, ISpatialReference spatialReference, bool simplify)
        {
            var multiPath = (IGeometryCollection)new PolylineClass();
            if (spatialReference != null)
                ((IGeometry)multiPath).SpatialReference = spatialReference;
        ...
        private static IGeometry BuildPolygon(WktText wkt, ISpatialReference spatialReference, bool simplify)
        {
            ...
            if (simplify)
            {				
				( ( ITopologicalOperator2 ) multiRing ).Simplify ( );
            }
        ...
        private static IGeometry BuildMultiPolygon(WktText wkt, ISpatialReference spatialReference, bool simplify)
        {
            var multiRing = (IGeometryCollection)new PolygonClass();
            if (spatialReference != null)
                ((IGeometry)multiRing).SpatialReference = spatialReference;
        ...
        private static IGeometry BuildMultiPatch(WktText wkt, ISpatialReference spatialReference, bool simplify)
        {
        ...
        private static IGeometry BuildGeometryCollection(WktText wkt, ISpatialReference spatialReference, bool simplify)
        {
            ...
            {
                var geom = BuildGeometry(geomToken.ToString(), spatialReference, simplify);
        ...
        private static ISpatialReference CreateDefaultSpatialReference()
        {
			...
			IGeographicCoordinateSystem spatialReference = spatialReferenceFactory.CreateGeographicCoordinateSystem ( ( int ) esriSRGeoCSType.esriSRGeoCS_WGS1984 );
