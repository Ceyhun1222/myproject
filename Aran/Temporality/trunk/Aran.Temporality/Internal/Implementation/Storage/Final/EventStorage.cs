...
{
    internal abstract class EventStorage<T, TKeyType> : CachedEventStorage<T, TKeyType>
        where T : class //T is feature type
    {
...
#warning implement it
        ...
        private List<FeatureDependencyException> CheckRelatedFeatures(AbstractEvent<T> myEvent, IEnumerable<KeyValuePair<int, List<IFeatureId>>> relatedFeatures)
        {
        ...
        private readonly ILogger _logger = LogManager.GetLogger(typeof(EventStorage<T, TKeyType>));

        #endregion

        #region Overrides of AbstractEventStorage<T>

        public override void SaveEventInternal(AbstractEvent<T> myEvent)
        {
...
#warning Implement feature dependency
            ...
            _logger.Debug("Saving event meta to storage");

            var meta = new OffsetEventMetaData<TKeyType>(myEvent)
            {
                RelatedFeatures = relatedFeatures
            };

            meta.Offset = Repository(myEvent.WorkPackage).Add(myEvent.Data, meta);

            myEvent.Data = Repository(myEvent.WorkPackage).Get(meta.Offset, meta.FeatureTypeId);

            SaveDataConsistency(myEvent);
        ...
        protected abstract void SaveDataConsistency(AbstractEvent<T> savedData);

        public override IList<AbstractEvent<T>> GetEventsAffectingDateInterval(IFeatureId featureId, DateTime fromDate, DateTime? toDate, bool andOneMore = false, Projection projection = null)
        {
            EventMetaSet<OffsetEventMetaData<TKeyType>> metaset = MetaCacheActual.GetByFeatureId(featureId);
            ICollection<OffsetEventMetaData<TKeyType>> subset = metaset.GetViewBetween(fromDate, toDate, andOneMore);
            //manipulation with subset here

            //if there is no Data return empty list
            if (subset.Count == 0) return new List<AbstractEvent<T>>();

            subset = subset.OrderBy(t => t.Offset).ToList(); //for better performance

            var wpEvents = Repository(featureId.WorkPackage);

            var datas = wpEvents?.Get(subset.Select(x => x.Offset).ToList(), featureId.FeatureTypeId, projection);
            return datas?.Zip(subset, GetEventFromData).ToList() ?? new List<AbstractEvent<T>>();
        ...
        public override AbstractEvent<T> GetEventByVersion(IFeatureId featureId, TimeSliceVersion version)
        {
            ...
            return GetEventFromData(Repository(featureId.WorkPackage).Get(metaitem.Offset, metaitem.FeatureTypeId), metaitem);
        }
        ...
        public override bool CommitWorkPackage(int workpackage)
        {
            ...
            List<OffsetEventMetaData<TKeyType>> savedWorkPackageMeta = new List<OffsetEventMetaData<TKeyType>>();
            List<OffsetEventMetaData<TKeyType>> savedCancelledWorkPackageMeta = new List<OffsetEventMetaData<TKeyType>>();
            ...
            foreach (var pair in currentActualWorkPackageMeta)
            {
                ...
                SortedList<Guid, EventMetaSet<OffsetEventMetaData<TKeyType>>> defaultActualFeatureTypeData = null;
                defaultActualWorkPackageMeta?.TryGetValue(featureType, out defaultActualFeatureTypeData);

                SortedList<Guid, EventMetaSet<OffsetEventMetaData<TKeyType>>> defaultCancelledFeatureTypeData = null;
                defaultCancelledWorkPackageMeta?.TryGetValue(featureType, out defaultCancelledFeatureTypeData);

                SortedList<Guid, EventMetaSet<OffsetEventMetaData<TKeyType>>> currentCancelledFeatureTypeData = null;
                ...
                foreach (var pair2 in pair.Value)
                {
                    ...
                    EventMetaSet<OffsetEventMetaData<TKeyType>> currentCancelledMetaSet = null;
        ...
        private void CommitFeature(IFeatureId featureId, Interpretation interpretation, EventMetaSet<OffsetEventMetaData<TKeyType>> currentActualMetaSet,
            EventMetaSet<OffsetEventMetaData<TKeyType>> currentCancelledMetaSet, List<OffsetEventMetaData<TKeyType>> savedWorkPackageMeta, List<OffsetEventMetaData<TKeyType>> savedCancelledWorkPackageMeta)
        {
        ...
        private void CommitCorrections(IFeatureId featureId, Interpretation interpretation, EventMetaSet<OffsetEventMetaData<TKeyType>> currentMetaSet,
            List<OffsetEventMetaData<TKeyType>> savedWorkPackageMeta, List<int> sequenceStarts, bool actual = false)
        {
        ...
        private void CommitNewSequence(IFeatureId featureId, Interpretation interpretation, EventMetaSet<OffsetEventMetaData<TKeyType>> currentMetaSet,
            int start, List<OffsetEventMetaData<TKeyType>> savedWorkPackageMeta, TimeSliceVersion version, bool actual = false)
        {
            string type = actual ? "actual" : "non-actual";
            foreach (var meta in currentMetaSet.Where(
        ...
        protected void MoveMetaToDefaultWorkPackage(OffsetEventMetaData<TKeyType> meta, int sequenceStart = 0)
        {
            if (meta == null) return;
            var metaByWorkPackage = Repository(meta.WorkPackage);
            var data = metaByWorkPackage?.Get(meta.Offset, meta.FeatureTypeId);
            if (data == null) return;
            ...
            var myEvent = GetEventFromData(data, meta);
        ...
        public override IList<AbstractEvent<T>> GetChangesInIntervalInternal(IFeatureId featureId,
                                                                             DateTime startDate,
                                                                             DateTime endDate)
        {
            ...
            if (byWorkPackage == null) return new List<AbstractEvent<T>>();


            var metaSets = new List<EventMetaSet<OffsetEventMetaData<TKeyType>>>();
            ...
            if (featureId.FeatureTypeId == 0 || featureId.FeatureTypeId == -1)
            {
                ...
                else
                {
                    ...
                    foreach (var wpData in byWorkPackage.Values)
                    {
                        EventMetaSet<OffsetEventMetaData<TKeyType>> metaSet;
            ...
            {
                //add specific type
                SortedList<Guid, EventMetaSet<OffsetEventMetaData<TKeyType>>> metaParent;
            ...
            if (metaSets.Count == 0) return new List<AbstractEvent<T>>();
            ...
            foreach (var featureMetaSet in metaSets)
            {
                ...
                var datas = Repository(featureId.WorkPackage).Get(subset.Select(metaData => metaData.Offset).ToList(), featureId.FeatureTypeId);
                var events = datas?.Zip(subset, GetEventFromData).ToList() ?? new List<AbstractEvent<T>>();
                                                                             ...
                                                                             DateTime endDate)
        {
            ...
            var featureTypeValues = new List<SortedList<Guid, EventMetaSet<OffsetEventMetaData<TKeyType>>>>();
            if (featureId.FeatureTypeId == 0 || featureId.FeatureTypeId == -1)
            {
                //add all
            ...
            foreach (var featureTypeMetaSet in featureTypeValues)
            {
                ...
                {
                    ICollection<OffsetEventMetaData<TKeyType>> subset = featureMetaSet.GetViewBetween(startDate, endDate);
