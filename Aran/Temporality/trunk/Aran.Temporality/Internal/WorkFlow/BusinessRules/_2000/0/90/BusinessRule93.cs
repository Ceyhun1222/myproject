...
{
    internal class BusinessRule93 : AbstractBusinessRule
    {
        #region Overrides of AbstractBusinessRule

        public override bool CheckChild(object obj)
        {
            var item = obj as SafeAltitudeAreaSector;
            if (item == null) return false;
            //"Each UomDistanceType should be 'KM' or 'M' or 'CM'";
            if (item.BufferWidth!=null)
            {
                return item.BufferWidth.Uom == UomDistance.M ||
                    item.BufferWidth.Uom == UomDistance.CM ||
                       item.BufferWidth.Uom == UomDistance.KM;
            }
            return true;
        }
        ...
        {
            return typeof (SafeAltitudeAreaSector);
        }
