...
namespace MapEnv
{
    internal class AimFLGlobal
    {
        ...
        public static readonly string ShapeFileDirectory = Globals.TempDir + @"\SHF\";

        private static Dictionary<string, IWorkspace> _openedWorkspaceDict = new Dictionary<string, IWorkspace>();
        ...
        private static void SimpleLayerBuilderForm_FinishClicked_MultiFeature(object sender, EventArgs e)
        {
            ...
            Globals.MainForm.AddAimSimpleShapefileLayer(aimLayer, true, true);
        }
        ...
        private static string CreateShapeFile(string layerName, GeometryType geomType, List<ShapeFileRow> dataList)
        {
            ...
            {
                var pShp = CreateGeometry(row.Geom, row.ZValue, row.ZMin, row.ZMax);
        ...
        private static IntPtr CreateGeometry(Geometry geom, double zValue, double zMin, double zMax)
        {
            switch (geom.Type)
            {
                ...
                case GeometryType.MultiPolygon:
                    {
                        ...
                        {
                            paPartType[indexPaPartType++] = ShapeLib.PartType.OuterRing;
                            ...
                            for (int j = 0; j < polygon.InteriorRingList.Count; j++)
                            {
                                paPartType[indexPaPartType++] = ShapeLib.PartType.InnerRing;
                ...
                case GeometryType.MultiLineString:
                    {
                        ...
                        if (zMin != 0 && zMax != 0 && adfZ.Length == 2 &&
                            double.IsNaN(adfZ[0]) && double.IsNaN(adfZ[1]))
                        {
                            adfZ[0] = zMin;
                            adfZ[1] = zMax;
                        }
        ...
        private static double GetZValue(Aran.Aim.Features.Feature feature, out double zMin, out double zMax)
        {
            ...
            if (feature.FeatureType == FeatureType.VerticalStructure)
            {
                ...
                if (vs.Part.Count > 0)
                {
                    ...
                    {
                        var hp = vsPart.HorizontalProjection;

                        if (hp == null)
                            continue;

                        Aran.Aim.DataTypes.ValDistanceVertical elevVal = null;

                        switch (hp.Choice)
                        {
                            case VerticalStructurePartGeometryChoice.ElevatedPoint:
                                if (hp.Location != null)
                                    elevVal = hp.Location.Elevation;
                                break;
                            case VerticalStructurePartGeometryChoice.ElevatedCurve:
                                if (hp.LinearExtent != null)
                                    elevVal = hp.LinearExtent.Elevation;
                                break;
                            case VerticalStructurePartGeometryChoice.ElevatedSurface:
                                if (hp.SurfaceExtent != null)
                                    elevVal = hp.SurfaceExtent.Elevation;
                                break;
                        }
                        ...
                        var d = Aran.Converters.ConverterToSI.Convert(elevVal, elevVal.Value);
                        return d;
                    }
                }
            }
            else if (feature.FeatureType == FeatureType.Airspace)
            {
                    ...
                    airSp.GeometryComponent[0].TheAirspaceVolume != null)
                {
                    ...
                    if (av.LowerLimit != null)
                        zMin = Aran.Converters.ConverterToSI.Convert(av.LowerLimit, av.LowerLimit.Value);

                    if (av.UpperLimit != null)
                    {
                        zMax = Aran.Converters.ConverterToSI.Convert(av.UpperLimit, av.UpperLimit.Value);
                        return zMax;
                    }
                }
            }
            ...
            {
                var segmentLeg = feature as Aran.Aim.Features.SegmentLeg;

                if (segmentLeg != null)
                {
                    if (segmentLeg.LowerLimitAltitude != null)
                        zMin = Aran.Converters.ConverterToSI.Convert(segmentLeg.LowerLimitAltitude, segmentLeg.LowerLimitAltitude.Value);


                    if (segmentLeg.UpperLimitAltitude != null)
                    {
                        zMax = Aran.Converters.ConverterToSI.Convert(segmentLeg.UpperLimitAltitude, segmentLeg.UpperLimitAltitude.Value);
                        return zMax;
                    }
                }
            }
