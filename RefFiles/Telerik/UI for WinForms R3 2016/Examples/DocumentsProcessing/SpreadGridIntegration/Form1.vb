...
Namespace SpreadGridIntegration
    Partial Public Class Form1
        ...
        Private Function CreateWorkbook(grid As RadGridView) As Workbook
        ...
        Private Sub SetWidthOfColumns(worksheet As Worksheet, numberOfIndentColumns As Integer, columns As IList(Of GridViewColumn))
        ...
        Private Sub AddHeaderRow(worksheet As Worksheet, numberOfGroupDescriptors As Integer, columns As IList(Of GridViewColumn))
            ...
            Dim fill = New PatternFill(PatternType.Solid, ThemableColor.FromArgb(Me.headerRowColorBox.Value.A, Me.headerRowColorBox.Value.R, Me.headerRowColorBox.Value.G, Me.headerRowColorBox.Value.B), ThemableColor.FromArgb(0, 255, 255, 255))
            worksheet.Cells(0, 0, 0, numberOfGroupDescriptors + columns.Count - 1).SetFill(fill)
        End Sub

        Private Function AddDataRows(worksheet As Worksheet, startRowIndex As Integer, startColumnIndex As Integer, rows As IEnumerable(Of Telerik.WinControls.UI.GridViewRowInfo), columns As IList(Of GridViewColumn)) As Integer
            Dim rowIndex As Integer = 0
            ...
            Dim fill = New PatternFill(PatternType.Solid, ThemableColor.FromArgb(Me.dataRowColorBox.Value.A, Me.dataRowColorBox.Value.R, Me.dataRowColorBox.Value.G, Me.dataRowColorBox.Value.B), ThemableColor.FromArgb(0, 255, 255, 255))
            worksheet.Cells(startRowIndex, 0, startRowIndex + rowIndex - headerColumn, startColumnIndex + columns.Count - 1).SetFill(fill)
            Return rowIndex
        End Function

        Private Function AddGroupRow(worksheet As Worksheet, rowIndex As Integer, numberOfIndentCells As Integer, group As DataGroup, columns As IList(Of GridViewColumn)) As Integer
