...
Namespace Telerik.Examples.WinControls.GridView.Customize.CustomPainting
	Partial Public Class Form1
		...
		Protected Overrides Sub OnLoad(ByVal e As EventArgs)
			...
			def.ColumnGroups(0).Rows(0).Columns.Add(Me.radGridView1.Columns(0))
			def.ColumnGroups(0).Rows(0).Columns.Add(Me.radGridView1.Columns(1))

			For i As Integer = 0 To months.Length - 1
				def.ColumnGroups.Add(New GridViewColumnGroup(months(i) & "'11"))
				def.ColumnGroups(1 + i).Rows.Add(New GridViewColumnGroupRow())
				For j As Integer = 1 To 4
					def.ColumnGroups(1 + i).Rows(0).Columns.Add(Me.radGridView1.Columns(months(i) & "Week" & j))
				Next j
			Next i
